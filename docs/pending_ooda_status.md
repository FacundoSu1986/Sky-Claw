...
| MO2 executable path != MO2 instance base_directory | Cerrado | Resolución de `mods/` desde la metadata de la instancia (`ModOrganizer.ini` `[Settings] base_directory`/`mod_directory`, contrato PathSettings de MO2 verificado contra `src/settings.cpp`): `get_mo2_mods_path()` pasa de derivar `<exe>/mods` a la precedencia `MO2_MODS_PATH` → metadata de instancia (portable junto al exe, global bajo `%LOCALAPPDATA%\ModOrganizer`) → legacy portable → fail-closed; caso real cubierto: exe en `C:\Modding\ModOrganizer2` e instancia con `base_directory` en `G:\Modding\MO2\SkyrimSE`. La detección del ejecutable (`detect_mo2_path`/`AutoDetector`) y los constructores manuales `<raíz>/mods` de otras superficies quedan enumerados como deuda hermanada en `tests/test_path_resolution_service.py` (ancla) | Registrar la base de la instancia en las raíces del sandbox de `AppContext` para que los flujos de la GUI operen sobre el path resuelto | `test_path_resolution_service.py` |
