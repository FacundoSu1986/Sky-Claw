...
| MO2Controller monorraíz (grass y surface agente) | Abierto | — | #555 dejó `grass_runtime_deps` sobre `get_mo2_path()` deliberadamente: `MO2Controller` usa el MISMO root para `ModOrganizer.exe` (INSTALL, `launch_game`) y `profiles/<p>/modlist.txt`/`mods/<m>` (DATA/MODS), y `GrassProfileManager`/`overwrite_grass_dir` cuelgan de esa raíz única. En layout separado (`C:\...\ModOrganizer2` vs `G:\...\SkyrimSE`) el ritual grass muta el árbol de instalación; el PathValidator no lo corta porque install es raíz legítima del sandbox. El fix exige separar raíces dentro de `MO2Controller` (call sites: `app_context.py:937`, `grass_runtime_deps.py`, `grass_profile.py`) + broker/attestation (`vfs_attestation` construye `<root>/mods`, hoy fail-closed con evidencia ante `mod_directory` custom pero sin consumir el MODS_DIR declarado). **Bloqueante**: no declarar la familia split-brain (#552→#555) cerrada sobre un rig con instancia separada sin cerrarla | Issue #557; al cerrar: test end-to-end con topología install != data (estilo `TestRaizDeDatosDeInstanciaSeparada`) y extensión del ancla semántica a los constructores de `MO2Controller`; `test_grass_runtime_deps_provider.py`, `test_vfs.py` |
