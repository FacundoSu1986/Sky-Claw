...
| T-31 | Cerrado | — | Lock cross-process por `install_dir`/`mod_dir` en `ToolsInstaller` (8 rutas de instalación incl. `ensure_skse` + wiring en `app_context.py`; ancla de familia e introspección) | `test_tools_installer_lock_scoping.py`, `test_tools_installer_lock_contencion.py` |
