...
> reverificación integral del resto de la tabla.
>
> **Re-baseline parcial 2026-09-09 sobre `main` `5e5e9448` (#569):** cubre
> exclusivamente la redacción del blocker de staging externo por herramienta en
> las filas «Fronteras del handoff TexGen → DynDOLOD» y «DynDOLOD ya no se lanza
> tras una etapa TexGen fallida»: la decisión de lifecycle/ownership del work
> root externo (`external_work_root`) pasó de pendiente a documentada en
> [ADR 0011](adr/0011-dyndolod-external-work-root.md) y su
> [spec](design/specs/2026-09-09-dyndolod-external-work-root.md). **No** cierra
> el gate de aceptación de rig ni reverifica el resto de la tabla.
>
> **Re-baseline parcial 2026-09-10 sobre `main` `87706321` (#570):** cubre
> exclusivamente el cierre del gate de aceptación de rig de §2.9 (T5-v2,
> **PASS** — informe commiteado:
> [`validation/2026-09-10_t5v2_dyndolod_stage9.md`](validation/2026-09-10_t5v2_dyndolod_stage9.md)):
> la fila «Preset de TexGen desvía `OutputPath`» registra la precedencia medida
> en los dos binarios (decidir el mecanismo sigue abierto), y las filas
> «Fronteras del handoff TexGen → DynDOLOD» y «DynDOLOD ya no se lanza tras una
> etapa TexGen fallida» actualizan su blocker de staging externo: el gate de rig
> quedó cerrado y el bloqueo restante es P0. **No** es una reverificación
> integral del resto de la tabla y **no** cierra P0, PR-2, freshness ni #528.
>
> **Re-baseline parcial 2026-09-11 sobre `main` `a497fd51` (#571):** cubre
> exclusivamente el cierre de **P0** (prerrequisito de PR-2) en las filas
> «Fronteras del handoff TexGen → DynDOLOD» y «DynDOLOD ya no se lanza tras una
> etapa TexGen fallida». P0 entrega la CAPACIDAD del work root externo —
> preferencia `external_work_root`, binding v1 con creación single-winner
> cross-process, admisión con Known Folders por identificador, máquina de
> estados A–H enumerada, coordinación durable de etapa 9 y transición
> restart-only fail-closed (`sky_claw/local/tools/dyndolod_workspace.py`,
> `sky_claw/app/security/known_folders.py`) — y **no** la activa:
> `P0 CAPABILITY != PR-2 ACTIVATION`, con el `-o:` productivo verificado sin
> cambios sobre el argv real. **No** cierra PR-2 (no iniciado), ni los criterios
> 8–10 del checklist T5-v2 (sigue **PARCIAL 7/10**), ni la precedencia de
> presets, ni freshness, ni #528, ni T-PR2-23 (familia de recovery legacy, que
> pertenece a PR-2). **No** es una reverificación integral del resto de la tabla.
>
> **Re-baseline parcial 2026-09-11 sobre `main` `6dff0117` (#573):** cubre
> exclusivamente el cierre de **P2.0 — gate de ownership del root vivo**
> (bloqueante de activación de PR-2) en las filas «Fronteras del handoff
> TexGen → DynDOLOD» y «DynDOLOD ya no se lanza tras una etapa TexGen fallida».
> P2.0 agrega la mitad de la unicidad que P0.2 dejó fuera de alcance a
> propósito: la exclusividad del `WorkspaceResuelto` VIVO, no sólo la del
> registro durable. Lease larga `dyndolod-ownership-<clave>` (namespaced por
> `ResourceBinding.clave()`, identidad de owner por proceso), reutilizando
> `SnapshotTransactionLock` (heartbeat, `lease_lost`, `assert_owned`), adquirida
> dentro del boundary del lock corto y liberada en el shutdown de `AppContext`;
> `WorkspaceResuelto.assert_owned()` fail-closed para el snapshot viejo; dos
> procesos reales con cwd distinto demostrando que B rechaza OCUPADO mientras A
> conserva ownership, y readquisición tras muerte dura + expiración de lease con
> TODAS las validaciones P0 de transición intactas. **No** cierra PR-2 —P2.1
> (derivación + per-tool `-o:`), P2.2 (servicio + packaging) y P2.3 (recovery)
> siguen NO IMPLEMENTADOS, el `-o:` productivo no cambió—, ni los criterios
...
>
> **ESTADO CONSOLIDADO PR-2 (2026-09-13, post-merge) — los bloques anteriores
> de P2.1/P2.2/P2.3 quedan HISTÓRICOS:** #580 mergeó por squash en `main`
> `1801a1ba`; el gate de lanzamiento quedó **CLOSED — PASS** con el rig real del
> candidato completo (T5-v2 **10/10**, informe commiteado:
> `validation/2026-09-13_pr580_real_rig/final-report.md`) y P2.1/P2.2/P2.3
> quedaron **COMPLETE**. Los "DRAFT / REOPENED / PARCIAL 7/10 / NOT READY TO
> MERGE" de esos bloques describen el estado del CANDIDATO en su momento, no el
> vigente.
>
> **Re-baseline parcial 2026-09-13 sobre `main` `1801a1ba` (#580) — PR-3:** cubre
> exclusivamente la **retirada de la comparación pre/post de firma del artefacto**
> en `dyndolod_runner.py` (PR-3). Con el root completo born-empty, la exclusividad
> por herramienta y el fence de ownership que PR-2 dejó implementados y el rig
> real T5-v2 10/10 verificó
> (`validation/2026-09-13_pr580_real_rig/final-report.md`), el predicado ∃ ("el
> artefacto cambió") quedó arquitectónicamente redundante y se eliminó
> (`_firma_de_veredicto`, `_firmas_de_salida`, la comparación pre/post y los
> errores exclusivos de "artefacto rancio"). **La atribución del log por corrida
> se conserva** (`_firma_del_log`, `_validar_completitud_de_la_corrida`,
> `_digest_de_stream`): el log no vive dentro del root born-empty. El veredicto
> sigue exigiendo rc válido AND artefacto requerido presente en el subroot
> exclusivo AND completitud de ESTA corrida AND sin terminales de ESTA corrida.
...
| U-06 | Parcial | #375 para DynDOLOD; post-check de artefacto de BodySlide; veredicto por `Engine.log` de Pandora; post-check por LOG **+ gate de artefacto** de DynDOLOD/TexGen (binarios GUI sin stdout: ni el exit code ni la mera existencia alcanzan; desde PR-3 el subroot exclusivo nace vacío y la comparación pre/post de firma del artefacto quedó retirada, mientras la atribución del log por corrida permanece) | Post-check de Wrye Bash; criterio seguro para QuickAutoClean; **completitud** de la etapa 9: el gate de artefacto prueba que la salida existe y es estructuralmente válida, no que la corrida terminó. el rig 2026-08-10 midió que DynDOLOD persiste el `.esp` al final (t3 > t4), lo que abre la puerta a apretar el criterio con marcadores de completitud del log. **Esa medición no es auditable desde el árbol** — informe fuera del repo, una sola corrida — así que la fila NO la da por cerrada: es la premisa que T2 tiene que re-verificar con la fixture del log commiteada, no un hecho establecido. Los marcadores **ya están cableados** en los dos lanzadores y TexGen tiene el suyo (`TexGen completed successfully`): ver la fila «Clasificación del log de la etapa 9» para el estado vigente, que es donde vive ese contrato | tests de cada runner, `test_bodyslide_postcheck_artefacto.py`, `test_pandora_runner.py`, `test_dyndolod_service.py` (post-check por log, artefacto y ancla enumerativa de la familia) |
...
| Contrato de veredicto de éxito | Parcial | Veredicto por `Engine.log` de Pandora; cruce con errores parseados en el call site de xEdit; ancla que enumera. **LOOT (PR-2):** `CHANGED`/`NO_CHANGE`/`FAIL` tipado — rc 0 sólo es éxito con testigo de ejecución fresco (`LOOTDebugLog.txt` recreado por `LootState`, loot/loot 0.29.1 `loot_state.cpp:105-106`) y estado del load order observable; CHANGED vs NO_CHANGE por comparación semántica (libloadorder 18.8.1, incluida la semántica exacta de `str::lines()`), no por mtime. Settings gestionados en el data root aislado: `useNoSortingChangesDialog = false`, `enableLootUpdateCheck = false` (un release nuevo o un error de red agregan un mensaje de error que cancela el auto-sort, `main_window.cpp:2830-2846/3062-3102`) y `lastVersion` = versión atestiguada del `VERSIONINFO` de `LOOT.exe` (sin ella, "First-Time Tips" `main_window.cpp:366-368` bloquea todo root nuevo y toda corrida tras actualizar LOOT); versión no atestiguable → `PRECONDITION_FAILED` sin lanzar. Resultado del worker USVFS fuera de contrato → `PROTOCOL_ERROR` | Pandora: verificar contra rig real qué severidad usa para la reversión de un nodo inválido — ver nota abajo. **LOOT: rig real R1–R5 pendiente** (CHANGED, NO_CHANGE, GUI ya abierta/mutex, fallo de sort, timeout/modal) más R0: root nuevo sin "First-Time Tips" y `lastVersion` = versión del binario. **Precondición de atribución:** el `--loot-data-path` debe ser de uso exclusivo de una invocación; `locks.db` relativo al cwd (`app_context.py:75`) no excluye a dos procesos Sky-Claw lanzados desde cwd distintos — hasta cerrarlo, un solo proceso por instancia MO2. `updateMasterlist` (red antes de cada sort; su fallo abre un modal) queda para PR-3 | `test_contrato_veredicto_de_exito.py` (inventario congelado en vacío), `test_pandora_runner.py`, `test_xedit_service.py`, `test_loot_verified_outcome.py` |
...
| MO2 executable path != MO2 instance base_directory | Cerrado | Resolución de `mods/` desde la metadata de la instancia (`ModOrganizer.ini` `[Settings] base_directory`/`mod_directory`, contrato PathSettings de MO2 verificado contra `src/settings.cpp`): `get_mo2_mods_path()` pasa de derivar `<exe>/mods` a la precedencia `MO2_MODS_PATH` → metadata de instancia (portable junto al exe, global bajo `%LOCALAPPDATA%\ModOrganizer`, o deferencia por `MO2_PATH` explícito de datos) → legacy portable → fail-closed; misma fuente para `resolve_modlist_path` (sin split-brain); path final exige `is_dir()` (rechaza archivo). **Wiring de producción cerrado**: `AppContext._construir_raices_sandbox` registra la base de instancia (vía `_raiz_datos_instancia_para_sandbox`, trust boundary explícito: canonicalizar, excluir drive root, exigir `is_dir`); `PathValidator` resultante permite que `get_mo2_mods_path`/`resolve_modlist_path` operen. **Separación INSTALL/DATA/MODS/PROFILE cerrada en #555 (superficie de lectura)**: `get_mo2_path()` es solo INSTALL ROOT con capability gate `is_dir` real sobre el hint inyectado, `get_mo2_instance_data_root()` deriva la raíz de datos de la metadata, los destinos de escritura respetan `mod_directory` por severidades separadas (`get_mo2_mods_path` / `_best_effort` / `_para_destino`), un hint inválido NO degrada a auto-detección (`has_explicit_mo2_install_selection`), el raw del hint se sanitiza igual que el del entorno, y una ancla semántica por AST fuerza a clasificar la raíz que necesita todo consumer nuevo — la superficie MUTANTE monorraíz (`MO2Controller`/grass) sigue abierta en la fila siguiente. La detección del ejecutable (`detect_mo2_path`/`AutoDetector`) y los constructores manuales `<raíz>/mods` de otras superficies siguen enumerados como deuda hermanada en `tests/test_path_resolution_service.py` (ancla) | — | `test_path_resolution_service.py` (incluye `TestWiringProductivoAppContext`, `TestRaizDeDatosDeInstanciaSeparada`, `TestAnclaSemanticaDeRaicesMo2`) |
| MO2Controller monorraíz (grass y surface agente) | Abierto | — | En progreso / implementación integrada en #552, pendiente merge final | Issue #557: MO2Controller split en install_root, data_root y mods_dir (rechaza combinaciones parciales, fail-closed); VFS manifest y broker actualizados a protocolo v2 con data_root y mods_dir explícitos; ritual de grass (GrassProfileManager y GrassRuntimeDepsProvider) consume get_mo2_instance_data_root() y get_mo2_mods_path_para_destino() (write-safe, fail-closed ante mod_directory inválido, MODS_DIR_UNAVAILABLE jamás llega a controller/profile manager); test end-to-end con topología install != data != mods (TestRaizDeDatosDeInstanciaSeparada), anclas semánticas de getters y de constructores manuales de mods actualizadas; test_grass_runtime_deps_provider.py, test_grass_profile.py, test_vfs.py, test_path_resolution_service.py |
...
| Fronteras del handoff TexGen → DynDOLOD | Cerrado | Las TRES, fail-closed. El layout físico correcto destapó tres agujeros que el candidato roto contenía por accidente, y los tres se cierran negándose a afirmar lo que no se puede probar. **A — ownership:** la raíz administrada la comparten las dos herramientas, así que empaquetarla entera absorbía `root/textures` dentro de "DynDOLOD Output"; ahora la raíz NO es una unidad empaquetable (la detección sigue reconociéndola: se prohíbe el ownership, no el hallazgo), y la regla es abstracta —*namespace compartido ≠ namespace empaquetable*— no un filtro por el nombre `textures`. **B — propiedad:** el staging de TexGen entra al move-aside del servicio ANTES de lanzar, así que nace vacío y "lo que hay adentro" pasa a ser "lo que esta corrida generó"; la frescura era un predicado ∃ y nunca pudo ser ∀. El destino exacto quedó declarado en `rollback_reconciler` para que su residuo no quede huérfano tras una muerte dura. **C — visibilidad:** empaquetar en `mods/` no demuestra nada sobre lo que DynDOLOD lee, porque corre standalone contra el `-d:<Data>` físico; antes del spawn se exige que TODO el staging esté visible ahí con los mismos bytes (identidad física o sha256, sin muestreo), y si no, DynDOLOD no se lanza | **No se materializa nada automáticamente**: el gate mide y falla cerrado, la materialización sigue siendo del operador — pero desde el fix F1 el corte por visibilidad PRESERVA `mods/TexGen Output` (el resto de la corrida revierte igual), así que lo que hay que desplegar sobrevive y la continuación con `run_texgen=False` lo verifica byte a byte contra el `Data` antes de lanzar DynDOLOD. Antes el rollback borraba justo ese artefacto y el default de la GUI quedaba sin salida. La TX queda PENDIENTE, nunca ROLLED_BACK, y el registro nombra el directorio vivo. `mutation_coverage_complete` sigue en `False` (el dir del exe y el temp quedan fuera del move-aside). Los subroots exclusivos por herramienta —el fix definitivo de A, que elimina la clase en vez de la instancia— **se implementaron en PR-2 (#580, squash `1801a1ba`)**, con el rig post-PR-2 en T5-v2 **10/10** (informe commiteado: `validation/2026-09-13_pr580_real_rig/final-report.md`); la decisión de lifecycle/ownership del work root externo quedó documentada en [ADR 0011](adr/0011-dyndolod-external-work-root.md), y PR-3 retiró la comparación pre/post de firma del artefacto. | `test_dyndolod_service.py` (T-A + ancla de política, T-B, T-B-rollback ×2, T-C, T-C-positive, T-C-stale), `test_rollback_reconciler.py` (destino exacto), `test_borrado_recursivo.py` (política de medición del gate) |
| DynDOLOD ya no se lanza tras una etapa TexGen fallida | Cerrado | Antes, un TexGen que fallaba —por proceso, por excepción, sin output atribuible o con su empaquetado roto— dejaba el pipeline en `success=False` pero DynDOLOD arrancaba igual: 30+ min de corrida y un "DynDOLOD Output" recién empaquetado que el rollback del servicio retiraba. El gate del spawn ahora corta ANTES de `run_dyndolod` cuando `run_texgen=True` y la cadena no quedó completa: veredicto TexGen válido → output contractual → packaging exitoso → visibilidad demostrada. `dyndolod_result` queda en `None` (evidencia honesta de que la herramienta no se ejecutó), `needs_deployment` sigue reservado al corte por visibilidad tras packaging exitoso (F1), y `texgen_packaging_attempted` conserva su semántica (el boundary de reemplazo del artifact se cruzó o no). `run_texgen=False` conserva el camino de Resume/sin-TexGen: con "TexGen Output" preservado se re-verifica contra el `Data`; sin él, DynDOLOD corre como antes | **Abiertos, no tocados por este PR:** external per-tool staging (subroots exclusivos; la decisión de lifecycle/ownership está documentada en [ADR 0011](adr/0011-dyndolod-external-work-root.md) y su prerrequisito **P0 quedó implementado el 2026-09-11** —preferencia, binding, admisión y coordinación durable, sin tocar `-o:`— **y su gate de activación P2.0 (ownership vivo del root) el 2026-09-11**, así que P2.1 quedó habilitado y **PR-2 mergeó en #580** (squash `1801a1ba`), con el rig post-PR-2 en T5-v2 **10/10** (informe commiteado: `validation/2026-09-13_pr580_real_rig/final-report.md`)); el gate de aceptación de §2.9 quedó cerrado por T5-v2); preset `OutputPath=` (sección propia arriba); volcado del log real completo contra la taxonomía; freshness cleanup (hecho en PR-3: retirada la comparación pre/post; la atribución del log se conserva); #528 (UIA output gate, downstream de este trabajo); rigs finales | `test_dyndolod_service.py` (T1–T8 del fail-stop: success=False, excepción, output None, packaging fallido, visibilidad, happy path, `run_texgen=False` con/sin mod preservado, efectos laterales sobre `mods/`, y el corte atravesando `DynDOLODPipelineService.execute` por el camino estándar) |
| Recovery de arranque de los ACTIVE_TARGET externos (PR-2 P2.3) | Cerrado | #580 (squash `1801a1ba`): antes de PR-2 el residuo move-aside de los roots externos no se barría: una muerte dura entre el move-aside y su restauración dejaba `<family>/<Tool>.rollback-*` huérfano — la única copia del staging previo. P2.3 declara los dos roots crudos por herramienta como ACTIVE_TARGET del productor `dyndolod`, mantiene el `LEGACY_RECOVERY_ONLY_TARGET` en su propio productor recovery-only con el mismo lock, e inyecta las raíces desde el `RegistroDeRootActivo` (root activo + `desde` de una transición pendiente) porque el barrido corre antes de resolver el workspace. Los casos de crash entre move-aside/mkdir/copia se cubren por target y la familia A–H del legacy se enumera caso por caso. | —; rig post-PR-2 T5-v2 10/10 (informe commiteado: `validation/2026-09-13_pr580_real_rig/final-report.md`) | `test_rollback_reconciler.py` (T-PR2-06 y T-PR2-23), `test_dyndolod_workspace.py` (helper de registro del arranque) |
| Deuda manual de Dawnguard en QuickAutoClean (SOP `local/AGENTS.md` §2.1) | Parcial | PR #637 (rama actual, sin merge): `XEditPipelineService.quick_auto_clean` sostiene las DOS pasadas automáticas y, cuando `Dawnguard.esm` entra en `cleaned`, **declara la deuda manual** como campo estructurado `manual_pending` (y además en `logs`) en vez de reportar el master como limpio; `CELDAS_MANUALES_DAWNGUARD` se contrasta contra el texto del SOP por test (editar uno sin el otro rompe el ancla), y el camino de la estrategia (`QuickAutoCleanStrategy`, dispatcher de orquestación/GUI) quedó anclado como **pass-through** para que el campo no desaparezca justo donde el operador lee el resumen. No cierra el «criterio seguro para QuickAutoClean» de `U-06` (post-check del artefacto de la corrida), que sigue abierto | **Integración con el ritual**: ninguna superficie bloquea el uso del master hasta que el operador confirme la limpieza de esas celdas, y el handoff no se persiste (un reinicio pierde el aviso). El smoke real sigue bloqueado por rig humano (fila «Smoke real de QuickAutoClean») | `test_xedit_quick_clean.py` (`test_exito_de_dawnguard_declara_la_deuda_manual_del_sop`, `test_las_celdas_manuales_de_dawnguard_coinciden_con_el_sop`, `test_pasadas_qac_congela_el_special_case_de_dawnguard`, `test_exito_sin_dawnguard_no_declara_deuda_manual`) y `test_quick_auto_clean_strategy.py` (identidad del dict) |

<!-- markdownlint-enable MD013 -->

## TexGen: preset persistido puede desviar `OutputPath` fuera del staging administrado

> **Estado:** `OPEN / FOLLOW-UP — precedencia medida en los DOS binarios (T5-v2
> 2026-09-10, informe commiteado: validation/2026-09-10_t5v2_dyndolod_stage9.md);
> fix/mechanism todavía sin diseñar`. **Prioridad sugerida:** P1, antes de
> automatizar completamente la GUI de TexGen.
>
...
**Actualización T5-v2 (2026-09-10, informe commiteado:
`validation/2026-09-10_t5v2_dyndolod_stage9.md`):** la asimetría quedó observada.
Con `Presets\DynDOLOD_SSE_Default.ini` rancio presente al lanzar por el runner,
el wizard simple mantuvo el campo Output en el root del `-o:`; al entrar en
**Advanced** el campo pasó al `OutputPath` del preset; el operador corrigió
únicamente el campo Output y las escrituras fueron al root administrado (cero
archivos en el decoy). TexGen replicó su auto-carga (campo = `OutputPath` del
preset al abrir) y con la corrección asistida escribió solo en el root
administrado. La precedencia está medida; lo que sigue abierto de este ítem es
decidir e implementar el mecanismo.
...
**Lo que Sky-Claw SÍ contiene hoy (mecanismo vigente post-PR-2/PR-3)** —
verificado leyendo `_post_check`, `_tiene_artefacto` y el gate de empaquetado en
`run_full_pipeline`: el servicio aparta el root **completo** de la herramienta
(`DirectoryRollback`), lo recrea vacío y verifica 0 entradas antes del spawn
(`_preparar_root_vacio`), con la lease de ownership viva; el runner revalida esa
precondición justo antes de spawnear. Si el preset desvía las escrituras, el
candidato administrado (`<tool_root>/textures` para TexGen) no recibe el
artefacto requerido: `_tiene_artefacto == False` → `success=False`, y **el
empaquetado de TexGen** no corre. Por diseño, esto descarta dos consecuencias
que una lectura apresurada del hallazgo sugeriría: un output previo **no puede**
pasar por fresco (el root nace vacío, no hay versión anterior adentro), y
Sky-Claw **no puede** empaquetar a MO2 una superficie distinta de la generada —
la corrida falla cerrada, no en silencio (hallazgo del revisor Codex en el PR
#463, verificado contra el código antes de aceptarlo — no se listan acá como
riesgos).

> **HISTÓRICO (pre-PR-3):** hasta PR-3 la contención se apoyaba en el gate de
> frescura del artefacto: el candidato administrado se firmaba antes y después de
> lanzar y, si las escrituras reales iban al path del preset, esa firma no
> cambiaba y el post-check lo marcaba "artefacto rancio" → `success=False`. PR-2
> volvió redundante esa comparación (born-empty del root completo) y PR-3 la
> retiró; el mecanismo vigente es el de arriba: ausencia del artefacto requerido
> en el candidato administrado.
...
las escrituras (el candidato administrado no contiene el artefacto requerido,
su packaging no corre) ahora corta ANTES del spawn: no hay corrida de DynDOLOD
ni mod recién empaquetado que el rollback deba retirar. El corte sigue sin resolver el
defecto de fondo (el `OutputPath` del preset manda sobre el `-o:`), que es lo
que este ítem persigue.

Consecuencias **derivadas, no reproducidas** en el rig, que ningún gate de
Sky-Claw cubre porque ocurren en el path del preset — fuera de cualquier
superficie que Sky-Claw mire:
...
- si el path del preset coincide con el staging de una corrida **de Sky-Claw**
  ...
  no lo esté mirando;
- el diagnóstico no nombra al preset: el veredicto sólo informa que el candidato
  administrado no contiene el artefacto requerido, mientras TexGen sí generó
  texturas —solo que en otro lugar—. Es un problema de diagnóstico/UX, no de
  integridad.
