...
| Clasificación del log de la etapa 9 | Parcial | T2: taxonomía de tres cajas (`MARCADORES_DE_COMPLETITUD`, `NO_FATALES_DEL_DOMINIO`, `PATRONES_TERMINALES` + `clasificar_log`), veredicto `rc == 0` **AND** artefacto fresco **AND** completitud **AND** sin terminales en los DOS lanzadores, decodificación utf-8→cp1252→`replace`, y SOP §2.9 punto 3 enmendado (el log ausente pasa de warning a fallo). El marcador además tiene que ser de ESTA corrida: el log se firma antes de lanzar y el marcador se busca sólo en los bytes que la corrida agregó, así que un marcador heredado no revive ni cuando el binario apendea | **Volcar el log real completo contra la taxonomía.** Las fixtures se armaron desde las CATEGORÍAS del §11.4 del informe, no desde las 121 ocurrencias: si alguna cae fuera de la lista de no-fatales, la regla fail-closed la clasifica terminal y una corrida buena sale roja. Requiere el log de rig, que no está en el árbol. Queda además un residual declarado: un log que cambia de mtime sin crecer se toma como truncado-y-reescrito, así que una corrida que no escriba NADA en su log hereda el marcador viejo; cerrarlo exige firmar el contenido por hash | `test_dyndolod_taxonomia_log.py` (dict congelado, tres cajas, caso aislado sin marcador, marcador rancio con y sin append + su contracara, los dos encodings y el byte que ningún codec acepta), `test_dyndolod_service.py` (gate de terminales aislado por lanzador); rig 2026-08-10 (informe fuera del repo) |
