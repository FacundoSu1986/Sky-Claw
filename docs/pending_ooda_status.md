...
| Contrato de veredicto de éxito | Parcial | Veredicto por `Engine.log` de Pandora; cruce con errores parseados en el call site de xEdit; ancla que enumera. **LOOT (PR-2):** `CHANGED`/`NO_CHANGE`/`FAIL` tipado — rc 0 sólo es éxito con testigo de ejecución fresco (`LOOTDebugLog.txt` recreado por `LootState`, loot/loot 0.29.1 `loot_state.cpp:105-106`) y estado del load order observable; CHANGED vs NO_CHANGE por comparación semántica (libloadorder 18.8.1), no por mtime; `useNoSortingChangesDialog = false` gestionado en el data root aislado | Pandora: verificar contra rig real qué severidad usa para la reversión de un nodo inválido — ver nota abajo. **LOOT: rig real R1–R5 pendiente** (CHANGED, NO_CHANGE, GUI ya abierta/mutex, fallo de sort, timeout/modal). Bloqueante conocido del rig: el modal "First-Time Tips" (`main_window.cpp:366-368`) aparece en todo data root sin `lastVersion` y sólo se persiste en `closeEvent` — un root nuevo necesita un primer cierre normal (o un follow-up que atestigüe la versión del binario y siembre `lastVersion`) | `test_contrato_veredicto_de_exito.py` (inventario congelado en vacío), `test_pandora_runner.py`, `test_xedit_service.py`, `test_loot_verified_outcome.py` |
