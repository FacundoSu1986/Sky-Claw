...
Fase de Autorización en el Helper (Elevado):
    ...
    7. Producir el FULL AUTHORIZED PLAN en AUTHORIZED_OPERATIONS (único productor) — secuencia normativa completa, GATE antes del primer MUTATING(1):
        ...
        7b. Escribirlo completamente: serialización canónica única (claves ordenadas, UTF-8), escritura de una pasada, cierre sin errores.
        7c. `FlushFileBuffers(file)`. Si `FlushFileBuffers(handle) == FALSE`: capturar `GetLastError()`, FAIL CLOSED — no se emite `MUTATING(1)`, 0 nodos mutados -> `ROLLBACK_REQUIRED` -> `ROLLING_BACK` (no-op) -> `ROLLED_BACK` (ruta declarada C3 de §20; `DurableJournalFlushError`, §24). Nunca se avanza como si el plan fuera durable.
        7d. Durabilidad de creación/directorio: flush en cadena de las entradas de directorio — `FlushFileBuffers(operations\<op_id>\)` (entrada del archivo) y `FlushFileBuffers(operations\)` (entrada del directorio nuevo si se creó en esta operación), cada directorio abierto con `FILE_FLAG_BACKUP_SEMANTICS` y derecho de escritura (`FlushFileBuffers` exige `GENERIC_WRITE`, doc. primaria §29); cualquier `FALSE` -> FAIL CLOSED igual que 7c.
