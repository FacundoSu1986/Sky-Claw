...
- **GP2-T24:** WRITABLE MAPPING CASE (§4.1.2). Parte 1 (fail-closed alcanzable): writer `GENERIC_WRITE` con `dwShareMode = FILE_SHARE_READ` (fila H2 de la HANDLE SHARE MATRIX) -> probe con `ERROR_SHARING_VIOLATION` -> `REFUSE_TO_APPLY` / `ROLLBACK_REQUIRED`. Parte 2 (limitación declarada con drift real): handle abierto con `FILE_SHARE_READ | FILE_SHARE_WRITE` + `CreateFileMapping(PAGE_READWRITE)` sobre él -> el probe no detecta el mapping (open exitoso); tras la mutación, el fixture **escribe un byte a través del mapping** en un nodo mutado y el test aserta que RV-2 (`TreeDigest`) detecta el drift y aborta **antes de `COMMITTED`** — la omisión de la defensa RV-2 hace fallar el test. Si el `CreateFileMapping` del fixture falla, el test falla explícitamente (setup error); nunca pasa en verde por la razón equivocada.
