...
| H6 | `GENERIC_READ` (sólo lectura, sin `FILE_WRITE_DATA`) | `FILE_SHARE_READ \| FILE_SHARE_WRITE \| FILE_SHARE_DELETE` | SÍ (concede todas las clases del probe, aunque su `dwDesiredAccess` sea read-only) | **open exitoso** | **Caso sin riesgo de mutación**: un reader read-only no posee `FILE_WRITE_DATA`; no puede escribir sobre el nodo y no puede ampliar el acceso de un handle ya abierto sin un nuevo open, que la DACL post-mutación deniega (`ACCESS_DENIED`). Sigue a `SetSecurityInfo`; no aplica la limitación de drift de H4 |

**Limitación declarada (crítica, no inventada):** NTFS no revoca los derechos concedidos a un handle ya abierto cuando se cambia la DACL; un writer que abrió con `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE` antes de la mutación **no es detectado por el probe** (fila H4: el probe tiene éxito porque el handle concedió todas las clases solicitadas) y conservará su handle con `GENERIC_WRITE`. La fila H6 (`GENERIC_READ` full-share) **no** es una limitación de mutación: un reader read-only no puede escribir y no puede ampliar su acceso sin un nuevo open denegado por la DACL post-mutación; es un caso sin riesgo (ver H6). La garantía v1 es, por tanto, más débil de lo que el texto previo sugería: GP2 **no afirma detección universal de writers**. Lo que GP2 v1 sí garantiza:
...
3. **Detección de drift de contenido** vía RV-2 (`TreeDigest`) cuando un writer no detectado (H4) escribió.
4. **Cero afirmación de exclusividad absoluta** post-`COMMITTED` para handles writer con todos los share modes concedidos (H4). La garantía completa post-`COMMITTED` requiere un módulo de enforcement distinto (no GP2 v1).
...
**Semántica de construcción (Win32, verificada contra documentación primaria):** `CreateFileMappingW` con `PAGE_READWRITE` exige que el handle subyacente posea **derechos de acceso** `GENERIC_READ | GENERIC_WRITE`; **no** exige un `dwShareMode` determinado del handle. El `dwShareMode` sólo condiciona aperturas posteriores del archivo por otros handles, no la creación del mapping. Por tanto:

- **Caso A — fail-closed materializable:** mapping `PAGE_READWRITE` construido sobre un handle abierto con `dwShareMode = 0` (o sin `FILE_SHARE_WRITE`) y `GENERIC_READ | GENERIC_WRITE` es válido. El probe del helper (clases de escritura, `dwShareMode = 0`) colisiona con ese handle -> `ERROR_SHARING_VIOLATION` -> `REFUSE_TO_APPLY` (0 nodos) / `ROLLBACK_REQUIRED` (>0 nodos).
- **Caso B — limitación declarada:** mapping `PAGE_READWRITE` sobre un handle con `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE` (todas las write-shares concedidas) -> el probe **no** lo detecta (open exitoso); la única defensa ante drift de contenido por ese mapping es RV-2 (`TreeDigest`). Análogo a la fila H4 de §4.1.1.

`GP2-T24` cubre ambos casos (ver §25): el caso A con desenlace contractual `ERROR_SHARING_VIOLATION`, el caso B con limitación declarada y drift real detectado por RV-2.
...
- **GP2-T23:** Oráculo de la HANDLE SHARE MATRIX del §4.1.1. El test enumera **literalmente las seis filas H1–H6** con su desenlace contractual por fila (sin parametrización parcial y sin mocks que sustituyan `CreateFileW`):
  ...
  - H4 (`GENERIC_WRITE`, `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE`) -> **open exitoso**; el test aserta el open con éxito, la limitación documentada (el writer conserva su handle) y que **el writer escribe un byte en un nodo mutado tras la mutación**; el test aserta entonces que RV-2 (`TreeDigest`) detecta el drift de contenido y aborta la transición **antes de `COMMITTED`**. Si RV-2 estuviera ausente o rota, el drift no se detectaría y el test fallaría (no puede pasar por la razón equivocada).
  - H5 (`GENERIC_READ` con share incompleto, `FILE_SHARE_READ`) -> `ERROR_SHARING_VIOLATION` -> `REFUSE_TO_APPLY`.
  - H6 (`GENERIC_READ` con share completo `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE`) -> **open exitoso**, caso sin riesgo: el test aserta el open con éxito y que una **reapertura posterior con `GENERIC_WRITE` es denegada** (`ACCESS_DENIED` por la DACL post-mutación); el reader read-only no puede escribir un byte (su handle no tiene `FILE_WRITE_DATA`) ni ampliar acceso, por lo que **no se exige drift** para este caso — el oráculo de drift con escritura real queda en H4 (writer) y GP2-T24 parte 2 (mapping).
...
- **GP2-T24:** WRITABLE MAPPING CASE (§4.1.2), dos partes con desenlace contractual por parte:
  - **Parte 1 (fail-closed alcanzable, Caso A):** handle abierto con `GENERIC_READ | GENERIC_WRITE` y `dwShareMode = 0`; `CreateFileMapping(PAGE_READWRITE)` sobre ese handle (construcción válida) -> el probe del helper falla con `ERROR_SHARING_VIOLATION` -> `REFUSE_TO_APPLY` (0 nodos) / `ROLLBACK_REQUIRED` (>0 nodos). Sin mock de `CreateFileW`; si el `CreateFileMapping` falla en el setup, el test falla explícitamente (setup error).
  - **Parte 2 (limitación declarada, Caso B):** handle abierto con `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE` + `CreateFileMapping(PAGE_READWRITE)` -> el probe no detecta el mapping (open exitoso); tras la mutación, el fixture escribe un byte a través del mapping en un nodo mutado y el test aserta que RV-2 (`TreeDigest`) detecta el drift y aborta **antes de `COMMITTED`** — la omisión de la defensa RV-2 hace fallar el test.
