...
- **GP2-T15:** Simulación de crash en cada estado del FSM (incluyendo crash entre mutación y registro con WAL respaldado por `FlushFileBuffers`, rechazo de UAC resultando en `ELEVATION_REJECTED`, fallo TGR/PPSC/binding resultando en `REFUSE_TO_PLAN`, y crash en `ARCHIVING_BACKUP`) recupera la acción segura correcta. Ancla AST/comportamiento: verifica que la escritura del journal usa `FlushFileBuffers` y que la simulación de crash entre `write()` y `flush()` resulta en `INDETERMINATE` (no en `COMMITTED` silencioso) — desenlace anclado contra la fila C4b de §20. Ancla además el **conjunto de estados del FSM** por igualdad literal (incluye `REFUSE_TO_PLAN` como terminal no-mutador), de modo que un nuevo desenlace no declarado en el diagrama §19.2 rompa el test.
...
- **GP2-T23:** Oráculo de la HANDLE SHARE MATRIX del §4.1.1. El test enumera **literalmente las seis filas H1–H6** con su desenlace contractual por fila (sin parametrización parcial y sin mocks que sustituyan `CreateFileW`):
  ...
  - H6 (`GENERIC_READ` con share completo `FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE`) -> **open exitoso**; mismo tratamiento de limitación que H4: tras la mutación el reader (que conserva su handle) abre/amplía su acceso o escribe un byte en un nodo mutado y RV-2 debe abortar antes de `COMMITTED`.
