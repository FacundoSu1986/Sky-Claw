    ...
    7. Producir el FULL AUTHORIZED PLAN en AUTHORIZED_OPERATIONS (único productor) — secuencia normativa completa, GATE antes del primer MUTATING(1):
        ...
        7d. Política de durabilidad Windows (honesta, sin semántica POSIX inventada):
            ...
            - `POWER_LOSS_DIRECTORY_ENTRY_DURABILITY_LIMITATION = DECLARED` (modelo de crash, coherente con §28 limitación 2):
              ...
              Consecuencia de diseño (no debilita B3): el plan completo se escribe, flushea y revalida SIEMPRE antes del primer `MUTATING(1)`; la pérdida del plan implica que la operación nunca cruzó `MUTATING(1)` — cero nodos mutados — y el desenlace es el declarado para operación sin mutación (§20), nunca un rollback sin fuente autoritativa.
...
- `OpenProcessToken` + `DuplicateTokenEx` (elevated helper extracts the coordinator's token; el acceso al objeto proceso se gobierna por el DACL check —MIC no lo sustituye—, ver §13.4) / Mandatory Integrity Control — *learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-openprocesstoken* · *learn.microsoft.com/windows/win32/api/securitybaseapi/nf-securitybaseapi-duplicatetokenex* · *learn.microsoft.com/windows/win32/secauthz/mandatory-integrity-control*
