...
Tras la verificación, **el probe se cierra** y se abre un **handle de mutación** independiente con `READ_CONTROL | WRITE_DAC | WRITE_OWNER` (contrato único normativo, idéntico en §1.3, §2.3 y §12.2; `WRITE_OWNER` es imprescindible para que el rollback restaure OWNER/GROUP con `SetSecurityInfo`) y `dwShareMode = FILE_SHARE_READ` (sin `FILE_SHARE_WRITE`, sin `FILE_SHARE_DELETE`). El binding por `(VolumeSerialNumber, FileId)` revalidado en cada paso del handle de mutación neutraliza la ventana TOCTOU entre probe y `SetSecurityInfo`.
...
4. **Soporte Pleno de Elevación Over-The-Shoulder (OTS):** Al residir el staging, el store autorizado y el TGR en `%ProgramData%`, el helper elevado puede leer el plan candidate, el TGR y escribir el plan autoritativo independientemente de si la cuenta administrativa ingresada en el diálogo UAC coincide con el usuario interactivo o es un administrador secundario de la máquina.
   - **Obtención del token del operador original (normativo, cierra el hueco OTS):** la post-verificación (§12.3/§21/§22) y la re-ejecución RV-2 del refresco del TGR (§11.4) deben correr bajo el **token efectivo no elevado del operador interactivo original** (para que `state == HARDENED` valga bajo `CURRENT_EFFECTIVE_UNELEVATED_TOKEN`), que en OTS **no** es la cuenta del diálogo UAC. Un proceso elevado sólo por UAC **no** posee ese token automáticamente. El diseño lo obtiene por una de dos vías, en este orden de preferencia:
     1. **Duplicación del token por el coordinador (preferida, sin privilegios extra):** el coordinador no elevado (que ES el operador interactivo) captura su propio token efectivo al iniciar la operación y **duplica ese handle al proceso helper** en el momento de lanzarlo (handle inheritance / `DuplicateHandle` hacia el helper). El helper usa ese token para `CreateProcessAsUserW` del verificador (§12.3). No requiere `SYSTEM` ni `SeTcbPrivilege`.
...
- `WTSQueryUserToken` (obtain the interactive operator's token; requires `SeTcbPrivilege`/SYSTEM) / `DuplicateHandle` (pass the coordinator's token to the helper) — *learn.microsoft.com/windows/win32/api/wtsapi32/nf-wtsapi32-wtsqueryusertoken* · *learn.microsoft.com/windows/win32/api/handleapi/nf-handleapi-duplicatehandle*
