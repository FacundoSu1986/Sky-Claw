...
- **Caso A — la ruta no existe:** se crea con owner + DACL canónicos (owner `SYSTEM`/`Administrators`, DACL protegida según la tabla de §11.3) en una sola operación privilegiada.
- **Caso B — la ruta ya existe:** un `CreateDirectory` que retorna `ERROR_ALREADY_EXISTS` **no basta** y el objeto preexistente **no se confía por defecto**. El bootstrap debe, en orden: (1) abrir el objeto **sin seguir reparse** (`CreateFileW` con `FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT`); (2) verificar el tipo (directorio, no archivo; en `GetFileInformationByHandleEx(FileAttributeTagInfo)`: `ReparseTag == 0` y `FileAttributes` sin `FILE_ATTRIBUTE_REPARSE_POINT`); (3) **rechazar reparse/junction/symlink** (fail-closed); (4) leer owner + DACL (`GetSecurityInfo`); (5) verificar que el owner pertenece al conjunto permitido (`SYSTEM`/`Administrators`); (6) normalizar de forma privilegiada owner + DACL al estado canónico (`SetSecurityInfo` con `OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION | PROTECTED_DACL_SECURITY_INFORMATION`); o (7) si no puede demostrar la adopción segura (tipo no verificable, owner no permitido, reparse detectado, o cualquier error de verificación), **FAIL CLOSED**: el bootstrap aborta y exige intervención del operador (el objeto atacante se elimina manualmente y se reprovisiona). El namespace atacante **nunca** es confiado.
