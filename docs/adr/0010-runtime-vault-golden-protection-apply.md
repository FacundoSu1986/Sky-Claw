...
9. **Rollback transaccional fail-closed con WAL durable:** restauración compensatoria ante cualquier fallo o inconsistencia intermedia con protocolo Write-Ahead Logging (WAL) y persistencia física obligatoria mediante `FlushFileBuffers(journal_handle)` antes de `SetSecurityInfo`, verificando el resultado de cada flush (`FALSE` -> fail-closed: nunca se ejecuta la mutación ni se avanza el FSM como si la escritura fuera durable; ver §12.2 pasos 4/7).
...
3. **Garantía de Quiescencia y Mutación Estrictamente Atada a Handle (Handle-Bound Mutation) — DOS handles normativos por nodo:**
   ...
   - **Limitación declarada:** NTFS no revoca los derechos concedidos en handles ya abiertos cuando se cambia la DACL, y el probe es una **instantánea**: detecta todo handle de datos preexistente en el momento del probe (el probe usa `dwShareMode = 0`, que por la semántica bidireccional de `CreateFileW` colisiona con todo handle con acceso de datos, sin importar el share mode que éste haya concedido — ver HANDLE SHARE MATRIX §4.1.1). La garantía v1 es: (a) **detección** de todo handle de datos preexistente en el probe y en el rerun pre-`ARCHIVING_BACKUP`; (b) **negación fail-closed** de la transición a `COMMITTED` si el probe o el rerun detectan writer; (c) **detección de drift de contenido** vía RV-2 (`TreeDigest`) si un writer o mapping abre después del rerun y escribe. **No se afirma exclusividad absoluta post-`COMMITTED`**: un proceso con derechos puede reabrir y escribir; la garantía completa requiere un módulo de enforcement distinto (no GP2 v1). **La misma limitación aplica a handles de control preexistentes** (`WRITE_DAC`/`WRITE_OWNER` sin clases de datos): el probe no puede detectarlos —la compartición sólo evalúa clases de datos (§4.1.1)— y el cambio de DACL no revoca los derechos ya concedidos a un handle abierto (§4.1.4); la garantía `HARDENED` queda acotada por `CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED` (§4.1.4).
...
10. **Store de Operaciones Profile-Independent con Autorización Privilegiada:** El almacén de operaciones y manifiestos se divide en dos capas bajo el directorio del sistema `%ProgramData%\Sky-Claw\runtime_vault`:
    ...
    - `operations\<op_id>\authorized_plan.json`: store protegido donde **únicamente el helper elevado** puede escribir tras validar físicamente el plan contra el registro de Golden Masters confiables y obtener la confirmación privilegiada de intención del operador. El `authorized_plan.json` es el **plan autoritativo COMPLETO**: incluye, además de los bindings del plan, la tabla íntegra de nodos (cada registro con `relative_path`, `node_kind`, `VolumeSerialNumber`, `FileId`, `pre_sd_bytes_b64`, `pre_sd_sha256`, `pre_sd_length`, `pre_dacl_protected_flag` y todos los bindings requeridos); se escribe y se flushea con la política de durabilidad Win32 soportada (`FILE_FLAG_WRITE_THROUGH` + `FlushFileBuffers` del archivo con verificación de éxito; la durabilidad de la entrada de directorio queda limitada según §9.2.8/§12.2 paso 7d) **antes del primer `MUTATING(K)`**; desde ese instante es la **única fuente de recuperación** (`RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY`; `UNTRUSTED_STAGING_AS_RECOVERY_SOURCE = NEVER`; ver §9.2.8 y §12.2 paso 7).
...
15. **Journal Transaccional Propio con WAL y FlushFileBuffers:** La máquina de estados persiste sus transiciones y el estado de cada nodo en `sky_claw/local/runtime_vault/protection_journal.py` (en `AUTHORIZED_OPERATIONS`), completamente desacoplado de `sky_claw/app/db/journal.py`. Antes de mutar cualquier nodo $K$, se registra durablemente la intención `MUTATING(K)` y se invoca `FlushFileBuffers` para cerrar la ventana de pérdida por caída de energía. El resultado de `FlushFileBuffers` es un **gate**: si retorna `FALSE`, `SetSecurityInfo` de ese nodo no se ejecuta y el FSM transiciona por la ruta declarada de rollback (`ROLLBACK_REQUIRED`; rollback vacío si 0 nodos mutados, ver §12.2 paso 4 y §24 `DurableJournalFlushError`).
...
2. La nueva apertura debe **conceder** (en su propio `dwShareMode`) las clases de acceso que el handle preexistente obtuvo; si la nueva apertura usa `dwShareMode = 0`, no concede nada -> `ERROR_SHARING_VIOLATION` contra **todo** handle preexistente que tenga acceso de datos de lectura/escritura/borrado (`FILE_READ_DATA`/`FILE_EXECUTE`, `FILE_WRITE_DATA`/`FILE_APPEND_DATA` o `DELETE`). Las clases de atributos/EA y de modificación de directorio (`FILE_WRITE_ATTRIBUTES`, `FILE_WRITE_EA`, `FILE_ADD_FILE`, `FILE_ADD_SUBDIRECTORY`, `FILE_DELETE_CHILD`) **no son clases de datos de compartición** y no disparan la dirección 2: un handle que sólo las posee no colisiona con el probe (doc. primaria `CreateFileW`: share mode 0 "prevents subsequent open operations ... if they request delete, read, or write access" y "access requests to attributes or extended attributes are not affected by this flag"; ver §29).
...
5. **Handles de control preexistentes** (`WRITE_DAC`/`WRITE_OWNER` sin clases de datos) abiertos con share mode generoso: **no** son detectables por el probe y **conservan** sus derechos concedidos tras el cambio de DACL — limitación declarada con política normativa en **§4.1.4** (`CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED`).
...
### 4.1.4 CONTROL HANDLE PREEXISTENCE (limitación normativa declarada)

**Escenario (realizable, verificado contra documentación primaria):** un Actor D dueño del Golden `UNPROTECTED` (el owner obtiene `WRITE_DAC` implícito, §8.2) abre —antes del primer probe de GP2 sobre el nodo— un handle solicitando **sólo clases de control/metadatos** (`WRITE_DAC` y/o `WRITE_OWNER`, sin ninguna clase de datos read/write/delete) con `dwShareMode` generoso. Entonces:

1. **El probe NO lo detecta:** la compartición de `CreateFileW` sólo se evalúa para clases de datos de lectura/escritura/borrado (§4.1.1; doc. primaria: share mode 0 "prevents subsequent open operations ... if they request delete, read, or write access" y "access requests to attributes or extended attributes are not affected by this flag"). Un handle cuyo *access mode* no contiene clases de datos no colisiona con el probe (share=0) en ninguna dirección, ni con el handle de mutación (clases de control, sin datos).
2. **El cambio de DACL NO revoca los derechos ya concedidos:** el modelo de seguridad de Windows evalúa el security descriptor **sólo al abrir** el handle; los cambios posteriores al SD "no tienen efecto alguno sobre las sesiones (handles) ya existentes" sobre ese objeto (Keith Brown, *MSDN Magazine*, "Exploring Handle Security in Windows" — archivo Microsoft, §29). El handle preexistente conserva `WRITE_DAC`/`WRITE_OWNER` concedidos en su máscara de acceso del handle.
3. **Consecuencia:** tras `COMMITTED`, Actor D puede invocar `SetSecurityInfo`/`SetKernelObjectSecurity` a través del handle preexistente y reescribir la DACL/owner, deshaciendo el blindaje **sin abrir ningún handle nuevo**. La post-verificación `HARDENED` (fresca en el momento de la verificación) no lo detecta porque la reescritura ocurre después; el Owner Rights ACE (`S-1-3-4`) neutraliza el `WRITE_DAC` implícito del owner en **aperturas nuevas**, no en handles ya abiertos.

**No existe una primitiva Win32 soportada** para enumerar los handles de otros procesos sobre un archivo junto con sus máscaras de acceso, ni para revocarlos: la enumeración vía `NtQuerySystemInformation(SystemHandleInformation)` es API nativa no documentada, racy e inadmisible como contrato; `RmGetList` es best-effort, no reporta máscaras de acceso por handle y no cubre todos los tipos de handle. Por tanto GP2 v1 **no** promete detectar ni excluir estos handles, y declara la política honesta:

```text
CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED
CONTROL_HANDLE_UNDETECTED_REMEDIATION  = NONE_IN_GP2_V1 (no existe primitiva Win32 soportada)
UNDETECTED_SUBSET = handles abiertos antes del primer probe del nodo cuyo access mode NO
                    contiene clases de datos read/write/delete y cuyo dwShareMode es generoso
DETECTED_SUBSET   = el mismo tipo de handle con dwShareMode restrictivo (share=0 o sin
                    FILE_SHARE_WRITE/FILE_SHARE_DELETE) colisiona con el probe en la
                    dirección 1 y es rechazado como ERROR_SHARING_VIOLATION (§4.1.3)
```

**Clasificación contractual de `HARDENED` respecto de capabilities concedidas antes de GP2:** `HARDENED` certifica el resultado de un `AccessCheck` **fresco**, ejecutado en el momento de la post-verificación bajo `CURRENT_EFFECTIVE_UNELEVATED_TOKEN` sobre todo el subárbol y el parent (§21; ADR 0009). **NO certifica** la ausencia de handles preexistentes —de control o de datos— que conserven derechos concedidos antes de GP2, ni exclusividad post-`COMMITTED` (§4.1.1, limitación 4). La garantía de que el owner no puede reescribir la DACL vale para **aperturas nuevas** (la Target DACL con Owner Rights `S-1-3-4` niega `WRITE_DAC` a toda apertura posterior, §8.2), no para handles ya abiertos. Un falso "security-green" contractual queda excluido: este ADR no afirma detección, exclusión ni revocación de handles de control preexistentes.

**Oracle documental:** `GP2-T48` — fixture que abre sobre un nodo un handle `WRITE_DAC | WRITE_OWNER` (sin clases de datos, `dwShareMode` generoso) antes del probe y aserta: (a) el probe **no** produce `ERROR_SHARING_VIOLATION` y la operación llega a `COMMITTED`; (b) a través del handle preexistente, `SetSecurityInfo` con una DACL permisiva **tiene éxito** post-`COMMITTED` (la protección puede deshacerse silenciosamente: esto verifica la limitación declarada, no una capacidad de mitigación); (c) el caso gemelo con `dwShareMode = 0` para el handle de control SÍ colisiona con el probe (dirección 1, §4.1.1). El mutante `M-C1` (afirmar que el probe detecta/excluye handles de control preexistentes, o que el cambio de DACL revoca derechos de handles abiertos) es detectado por `GP2-T48`.
...
Amenaza: Handle de control preexistente (WRITE_DAC/WRITE_OWNER sin clases de datos, share generoso) que sobrevive al hardening conservando derechos concedidos
Mitigación: NO existe mitigación en GP2 v1 (no hay primitiva Win32 soportada para enumerar/revocar handles ajenos con sus máscaras): limitación declarada `CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED` (§4.1.4). `HARDENED` se acota al AccessCheck fresco de post-verificación; el handle preexistente puede reescribir DACL/owner post-COMMITTED. Oracle: GP2-T48 / M-C1.
...
class NodeSecurityBackup:
    ...
    pre_dacl_protected_flag: bool       # (dacl_control_flags & SE_DACL_PROTECTED) != 0; persistido explícito para recovery sin re-parseo
...
8. **Fuente de recuperación durable dentro del trust boundary (normativa):** ANTES del primer `MUTATING(K)`, el helper escribe en `AUTHORIZED_OPERATIONS` el plan autoritativo **completo** (`authorized_plan.json`, secuencia normativa de §12.2 paso 7) que contiene, por nodo, todo lo necesario para la restauración exacta: `relative_path`, `node_kind`, `VolumeSerialNumber`, `FileId`, `pre_sd_bytes_b64`, `pre_sd_sha256`, `pre_sd_length`, `pre_dacl_protected_flag` y todos los bindings del plan. La secuencia de durabilidad (escritura completa con `FILE_FLAG_WRITE_THROUGH` + `FlushFileBuffers` del archivo con verificación de éxito + revalidación de digest/binding) es un **gate**: si falla, no se emite ningún `MUTATING(K)`. La durabilidad de la entrada de directorio NO se afirma como garantía Win32 (`POWER_LOSS_DIRECTORY_ENTRY_DURABILITY_LIMITATION = DECLARED`, §12.2 paso 7d/§28). Desde el primer `MUTATING(K)` vale:
   ```text
   RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY
   UNTRUSTED_STAGING_AS_RECOVERY_SOURCE = NEVER
   ```
   `UNTRUSTED_STAGING` (el `candidate_manifest.json`) es borrable/adulterable por un proceso no elevado y **nunca** es fuente de rollback; en la recuperación (§20) puede usarse como evidencia diagnóstica, no como única fuente de restauración. El respaldo archivado en `golden_backups` (§23.2) es la copia de evidencia persistida a partir de `ARCHIVING_BACKUP`; la fuente autoritativa durante toda la operación (rollback, recuperación de crash) es `authorized_plan.json`. Oráculo: `GP2-T49` (crash mid-mutation + staging eliminado/adulterado -> rollback exacto desde el PRE protegido) y mutante `M-B3` (§26).
...
**Bootstrap del namespace y política anti-squatting (normativa):** el subárbol `%ProgramData%\Sky-Claw\runtime_vault\` (incluidos `staging\`, `operations\`, `locks\`, `golden_backups\` y el archivo `trusted_goldens.json`) es **provisionado por un componente privilegiado definido por el diseño** (instalador ejecutado por `Administrators`/`SYSTEM`, o un servicio de Sky-Claw que arranca con `SeImpersonatePrivilege`/`SeAssignPrimaryTokenPrivilege` y se auto-eleva al primer uso). El ADR no asume que el namespace ya existe, y **tampoco asume que no existe**: un Actor D puede intentar **precrear el namespace antes del provisioning privilegiado** (`PRE_PROVISIONING_NAMESPACE_SQUATTING`) para plantar un trust root atacante. El bootstrap privilegiado trata cada ruta protegida (`Sky-Claw\`, `runtime_vault\`, `operations\`, `locks\`, `golden_backups\`, `staging\` y `trusted_goldens.json`) así:

- **Caso A — la ruta no existe:** se crea desde su nacimiento (`lpSecurityAttributes` con `SECURITY_DESCRIPTOR` inicializado con owner, group y DACL protegida canónicos) en una sola operación privilegiada (`CANONICAL_NAMESPACE_OWNER = "S-1-5-18"` / `LOCAL SYSTEM`, `CANONICAL_NAMESPACE_PRIMARY_GROUP = "S-1-5-32-544"` / `BUILTIN Administrators`, DACL protegida según la tabla de §11.3), eliminando cualquier ventana temporal `create -> harden`.
- **Caso B — la ruta ya existe:** un `CreateDirectory` que retorna `ERROR_ALREADY_EXISTS` **no basta** y el objeto preexistente **no se confía por defecto**. El bootstrap debe, en orden: (1) abrir el objeto **sin seguir reparse** (`CreateFileW` con `FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT`); (2) verificar el tipo (directorio, no archivo; en `GetFileInformationByHandleEx(FileAttributeTagInfo)`: `ReparseTag == 0` y `FileAttributes` sin `FILE_ATTRIBUTE_REPARSE_POINT`); (3) **rechazar reparse/junction/symlink** (fail-closed); (4) leer owner, group + DACL (`GetSecurityInfo`); (5) verificar que el owner pertenece al conjunto permitido (`SYSTEM` `S-1-5-18` o `Administrators` `S-1-5-32-544`); (6) normalizar de forma privilegiada owner, group + DACL al estado canónico (`SetSecurityInfo` con `OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION | PROTECTED_DACL_SECURITY_INFORMATION`, fijando group a `CANONICAL_NAMESPACE_PRIMARY_GROUP = "S-1-5-32-544"` `BUILTIN Administrators`; `PRESERVE_LIVE_PRIMARY_GROUP = NO`, `PRIMARY_GROUP_EQUALS_OWNER = NO`; comparaciones siempre por SID binario/canónico `S-1-5-32-544`, nunca por display name localizado); o (7) si no puede demostrar la adopción segura (tipo no verificable, owner no permitido, reparse detectado, o cualquier error de verificación), **FAIL CLOSED**: el bootstrap aborta y exige intervención del operador (el objeto atacante se elimina manualmente y se reprovisiona). El namespace atacante **nunca** es confiado.
- **Política canónica de Primary Group del Namespace (normativa):** `CANONICAL_NAMESPACE_PRIMARY_GROUP = "S-1-5-32-544"` (`BUILTIN Administrators`). Para todo objeto protegido provisionado o normalizado bajo `%ProgramData%\Sky-Claw\` (`Sky-Claw\`, `runtime_vault\`, `staging\`, `operations\`, `locks\`, `golden_backups\`, `trusted_goldens.json`), el Primary Group canónico es **siempre** `BUILTIN Administrators` (`S-1-5-32-544`). El valor NO depende del token que ejecuta el provisioning, del owner final del objeto, del primary group previamente observado, ni del nombre localizado de la cuenta.
- **Trust root (excepción dura):** un `trusted_goldens.json` **preexistente** es **siempre FAIL CLOSED** (paso 7 del Caso B): no existe forma de demostrar la proveniencia de su contenido (Actor D pudo plantar un TGR forjado con entradas de su agrado). El bootstrap sólo acepta un TGR creado por el propio provisioning privilegiado en el Caso A.
- **Ancestros relevantes:** `%ProgramData%\Sky-Claw\` y `%ProgramData%\Sky-Claw\runtime_vault\` se validan con el mismo procedimiento (Caso A/B) **antes** de crear o endurecer cualquier subdirectorio, de modo que un trust root atacante no sea heredado por herencia desde un ancestro comprometido.

Oráculo: `GP2-T50` — un usuario estándar precrea `%ProgramData%\Sky-Claw\runtime_vault\` (y, en variante separada, `trusted_goldens.json`) antes del bootstrap privilegiado -> el namespace es reclamado/normalizado de forma segura (Caso B.6) o el bootstrap se rehúsa (FAIL CLOSED, Caso B.7); el trust root atacante **nunca** es confiado (el helper nunca compara planes contra un TGR plantado). El mutante `M-N1` (bootstrap que continúa por `ERROR_ALREADY_EXISTS` sin verificar tipo/owner/reparse ni normalizar) es detectado por `GP2-T50`.

La rama `staging\<op_id>\` se crea al vuelo por el coordinador no elevado con permisos de escritura limitados al propio usuario; las demás ramas las crea exclusivamente el componente privilegiado.
...
| `locks/` (intermedio) | Componente privilegiado (instalador) | `FILE_ALL_ACCESS` (`0x001F01FF`) | `FILE_GENERIC_READ \| FILE_TRAVERSE` concedidos SÓLO sobre el objeto directorio (ACE de contenedor **sin flags de herencia hacia archivos**: los flags/scope de la ACE deben ser tales que **NINGUNA ACE con `FILE_READ_DATA` es heredada por `*.lock`**) — **SIN `FILE_READ_DATA` sobre los archivos de lock, SIN `FILE_WRITE_DATA`, SIN `DELETE`, SIN `WRITE_DAC`, SIN `WRITE_OWNER`, SIN `FILE_ADD_FILE`, SIN `FILE_ADD_SUBDIRECTORY`, SIN `FILE_DELETE_CHILD`** | Sólo el helper elevado puede crear `locks\<lock_key>.lock`. Coherente con la fila `locks/` (`LOCKS_STORE`): los derechos de la ACE del directorio no llegan a los `.lock` como `FILE_READ_DATA` (bloque normativo de locks bajo la tabla). |

**Contrato normativo de `locks/` (resuelve la ambigüedad de herencia entre las dos filas `locks/` de la tabla; sin conceder lectura sobre lock files):**

```text
LOCK_DIRECTORY_RIGHTS_FOR_AUTHENTICATED_USERS:
    directory-only LIST/TRAVERSE as required
    (ACE de contenedor sin herencia de FILE_READ_DATA hacia archivos)

LOCK_FILE_RIGHTS_FOR_AUTHENTICATED_USERS:
    NO FILE_READ_DATA
    NO FILE_WRITE_DATA
    NO DELETE
    NO WRITE_DAC
...
- Los lock files (`locks\<lock_key>.lock`) pertenecen al helper/provisioner; `Administrators`/`SYSTEM` = `FILE_ALL_ACCESS`.
...
`INDEPENDENT_PROVENANCE` es una categoría de autoridad, no una fuente disponible por defecto. P0.1 no aprueba ningún source concreto ni define aún sus trust roots/formato de validación; hasta que una fuente específica verifique cobertura completa y runtime, no se puede emitir ese modo. Una evidencia que sólo cubre Skyrim base no cubre el Golden modded completo. La fuente final de autoridad del TreeDigest completo se etiqueta `OPERATOR_TOFU` salvo que un descriptor/provenance bundle cubra el tree entero. Provenance parcial puede registrarse como evidencia suplementaria, pero nunca eleva silenciosamente el tree compuesto a `INDEPENDENT_PROVENANCE`.
...
**Boundary de runtime observation (Actor D):** la implementación actual de `verify_golden_master` sólo compara `observed_runtime` recibido como input; no descubre/remeasure el runtime y `GoldenMasterCandidate` también puede transportar ese valor. Por tanto el contrato futuro requiere un `FreshRuntimeObservation` read-only que reacquire el runtime desde el root físico ya revalidado **en cada** pass `OBSERVE` y `VERIFY`, bajo el token original. `expected_runtime` del receipt nunca se copia como `observed_runtime`; tampoco se aceptan campos runtime staged/caller-supplied sin esa nueva adquisición y binding a operation/nonce/root. Si no hay una fuente observada fresca, el pase no puede producir `OBSERVED`/`VERIFIED` y termina `REJECTED` sin TGR write. RV-2 sigue siendo el comparador; no se atribuye discovery de runtime a `verify_golden_master`.
...
- `critical_expectations_digest` es `SHA-256` de los bytes de una serialización canónica única de la lista confirmada: cada entrada tiene exactamente `rel_path`, `expected_digest` y `expected_size`; `rel_path` usa la normalización anti-traversal de `CriticalFileExpectation` y se rechazan paths normalizados duplicados; las entradas se ordenan por orden lexicográfico de los bytes UTF-8 de `rel_path`; `expected_digest` va en minúsculas y `expected_size` se serializa siempre (entero JSON no negativo, no boolean, o `null`). El JSON es un array UTF-8 sin BOM ni newline final, `ensure_ascii=False`, con campos de objeto ordenados lexicográficamente, separadores compactos `(',', ':')` y sin campos extra/espacios insignificantes. La lista vacía se serializa exactamente como `[]`, y su digest es `SHA-256(UTF-8("[]"))`; nunca se omite ni se sustituye por `null`. `critical_expectations=()` sigue siendo válido si el TreeDigest completo y `expected_runtime` están autorizados. Si no está vacía, la lista completa y su digest quedan ligados a la misma fuente que TreeDigest/runtime; las expectativas críticas son complementarias y nunca sustituyen al TreeDigest.
- El registro protegido de operación, ligado por `operation_id`, conserva el receipt inmutable exactamente como fue emitido, los valores normalizados de `critical_expectations` (incluida la lista vacía) para recomputar su digest, el resultado y bindings de cada observación/RV-2 ejecutado, la evidencia/source reference validada cuando aplique, y las entradas TGR exactas `before` (`ABSENT` o valor previo) y `after` (todos los campos). El helper persiste y revalida este registro protegido con `before` y `intended after` antes del replace TGR; si no puede hacerlo, no escribe el TGR. El resultado final se anexa después. Es un registro de auditoría de Golden Admission, no una transición del FSM GP2 ni WAL/rollback de GP2 apply (no habilita recovery/replay automático del TGR). Se conserva mientras la entrada `after` sea la actual; no se elimina esa evidencia sin migración equivalente.
- Para que la relación con el TGR sea auditable sin cambiar su schema: en un commit exitoso el helper escribe `registered_by = receipt.operator_sid` y genera `registered_at` como timestamp UTC ISO-8601 `Z` del registro con al menos precisión de microsegundos (no copia `admitted_at`); antes del replace el helper verifica que sea único entre registros del mismo root físico y, ante colisión, incrementa de a 1 microsegundo hasta obtener un valor no usado o falla cerrado antes del replace. Como el row TGR no contiene `expected_runtime` ni critical expectations, cualquier consumidor posterior que necesite esa autoridad debe resolver un único registro de operación cuyo receipt y entrada `after` coincidan exactamente con la fila TGR actual en identidad física, `canonical_root`, TreeDigest, policy, `registered_by` y `registered_at`; cero o múltiples matches -> fail closed, no se reconstruye runtime desde staging. Si el replace es ambiguo, se marca commit desconocido y se revalida el TGR protegido contra los valores `before`/`after` exactos; nunca se reporta `REJECTED` ni se afirma que el row previo quedó intacto. La enmienda no cambia el schema JSON del TGR.
...
| Runtime sustituto o runtime de otra fuente | `expected_runtime` queda bajo el mismo source/receipt que TreeDigest; un `FreshRuntimeObservation` re-adquiere `observed_runtime` desde el root físico revalidado en cada pass y lo liga a operation/nonce/root. No se acepta el runtime de staging, `GoldenMasterCandidate` ni copiar `expected_runtime`; ausencia de observer o mismatch -> `REJECTED`, cero TGR write. |
...
Fase de Autorización en el Helper (Elevado):
    ...
    7. Producir el FULL AUTHORIZED PLAN en AUTHORIZED_OPERATIONS (único productor) — secuencia normativa completa, GATE antes del primer MUTATING(1):
        7a. Construir `authorized_plan.json` = {operation_id, canonical_root, VolumeSerialNumber, root_file_id, TreeDigest, node_count, policy_version, staging_digest, timestamp PPSC, plan_digest} + `nodes[]`: la TABLA ÍNTEGRA de nodos con, por nodo, TODO lo necesario para la restauración exacta: `relative_path`, `node_kind`, `VolumeSerialNumber`, `FileId`, `pre_sd_bytes_b64`, `pre_sd_sha256`, `pre_sd_length`, `pre_dacl_protected_flag` y todos los bindings del plan (§9.1/§9.2.8). El contenido de `nodes[]` se deriva 1:1 del candidate_manifest YA VALIDADO en el paso 5 (mismos bytes revalidados contra el descriptor vivo).
        7b. Escribirlo completamente: serialización canónica única (claves ordenadas, UTF-8), escritura de una pasada (`WriteFile`) sobre un handle de archivo creado con `CreateFileW(..., FILE_FLAG_WRITE_THROUGH, ...)` — doc. primaria: `FILE_FLAG_WRITE_THROUGH` hace que las escrituras no pasen por cache intermedia y hace que NTFS flushee los metadata changes resultantes de procesar la request (§29) — y cierre sin errores.
        7c. `FlushFileBuffers(file_handle)` sobre el ARCHIVO `authorized_plan.json` (gate normativo). Si `FlushFileBuffers(handle) == FALSE`: capturar `GetLastError()`, FAIL CLOSED — no se emite `MUTATING(1)`, cero llamadas a `SetSecurityInfo`, 0 nodos mutados -> `ROLLBACK_REQUIRED` -> `ROLLING_BACK` (no-op) -> `ROLLED_BACK` (ruta declarada C3 de §20; `DurableJournalFlushError`, §24). Nunca se avanza como si el plan fuera durable.
        7d. Política de durabilidad Windows (honesta, sin semántica POSIX inventada):
            - `AUTHORIZED_PLAN_CONTENT_DURABILITY` (garantía Win32 documentada): el CONTENIDO del archivo queda durable mediante `FILE_FLAG_WRITE_THROUGH` + `FlushFileBuffers(file_handle) == TRUE` (§29).
            - `DIRECTORY_ENTRY_DURABILITY` = NO AFIRMADA. Win32 no documenta una primitiva soportada equivalente a `fsync(parent_directory)`: la página "Directory Handles" enumera de forma CERRADA las funciones que aceptan un directory handle y `FlushFileBuffers` NO está entre ellas (§29; `FlushFileBuffers` documenta "a handle to the open file"). Queda PROHIBIDO normar `FlushFileBuffers(directory_handle)` (sobre `operations\<op_id>\` o `operations\`), flush de volumen, `FSCTL_LOCK_VOLUME`, desmontaje, APIs NT no documentadas o hacks equivalentes.
            - `POWER_LOSS_DIRECTORY_ENTRY_DURABILITY_LIMITATION = DECLARED` (modelo de crash, coherente con §28 limitación 2):
                i.   Crash de proceso/helper (sin caída del OS): recovery por el FULL AUTHORIZED PLAN — el crash no invalida la entrada de directorio y el contenido flusheado del archivo es la garantía aplicable.
                ii.  Reinicio normal / crash del sistema: el contenido del archivo fue escrito y flusheado con las garantías Win32 soportadas; la entrada de directorio es metadata gestionada por el filesystem y NO se afirma un punto de sincronización de la entrada controlable por la aplicación.
                iii. Pérdida abrupta de energía / cache de hardware fuera del control de Windows: NO se promete atomicidad ni durabilidad absoluta de la directory entry (limitación declarada).
              Consecuencia de diseño (no debilita B3): el plan completo se escribe, flushea y revalida SIEMPRE antes del primer `MUTATING(1)`. La pérdida del plan NO implica que la operación nunca cruzó `MUTATING(1)`: la directory entry no es controlable por la aplicación (punto iii), de modo que una pérdida abrupta de energía posterior a un `MUTATING(K)` durable puede eliminar un `authorized_plan.json` correctamente escrito y flusheado. Modelo de recuperación normativo:
                PROCESS_HELPER_CRASH:
                    si authorized_plan.json es discoverable + válido
                    -> recovery exacto desde AUTHORIZED_OPERATIONS (§9.2.8).
                SYSTEM_CRASH_OR_REBOOT:
                    si authoritative plan + journal siguen discoverable + válidos
                    -> recovery según matriz §20.
                ABRUPT_POWER_LOSS_WITH_AUTHORITATIVE_PLAN_LOSS:
                    exact automatic rollback NOT GUARANTEED
                    staging fallback PROHIBITED
                    silent COMMITTED PROHIBITED
                Si existe CUALQUIER evidencia durable de que la transacción pudo
                comenzar (lock residual con phase != RELEASED, journal parcial,
                operations\<op_id>\ existente con evidencia MUTATING/MUTATED,
                SD actual diferente del PRE/target conocido, u otra evidencia
                transaccional durable) pero authorized_plan.json está missing /
                corrupt / unreadable / digest-invalid, el resultado obligatorio es:
                    INDETERMINATE
                    AUTO_ROLLBACK = NO
                    AUTO_COMMIT = NO
                    STAGING_AS_RECOVERY_SOURCE = NEVER
                    OPERATOR_INTERVENTION_REQUIRED = YES
                Prohibido: inventar PRE desde el filesystem actual; reconstruir PRE
                desde staging; declarar "operación nunca iniciada" sólo porque falta
                el authoritative plan (fila C4c de §20; GP2-T52 / M-P2).
        7e. Revalidar digest/binding: re-leer `authorized_plan.json` del disco y exigir `sha256(bytes canónicos) == plan_digest` y coincidencia del binding (operation_id, staging_digest, node_count). Si difiere -> FAIL CLOSED igual que 7c.
        7f. Desde este punto:
                RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY
                UNTRUSTED_STAGING_AS_RECOVERY_SOURCE = NEVER
            (el staging puede usarse como evidencia diagnóstica en recovery, §20, pero nunca como única fuente de rollback; ver §9.2.8).
        7g. Sólo entonces puede comenzar la mutación (primer `MUTATING(1)`, paso 4 del bucle de mutación).
...
Fase de Mutación por Nodo K en el Plan (en orden Bottom-Up):
    ...
    4. Write-Ahead Log (WAL) con FlushFileBuffers Obligatorio y VERIFICADO (gate normativo):
       Registrar en el journal de AUTHORIZED_OPERATIONS: MUTATING(K, pre_sd_sha256, pre_sd_protected_flag).
       if FlushFileBuffers(journal_handle) == FALSE:
           err = GetLastError()
           // GATE: la intención MUTATING(K) NO es durable -> SetSecurityInfo de K NO se ejecuta
           // y el FSM NO avanza como si la escritura fuera durable:
           //   - 0 nodos mutados  -> ROLLBACK_REQUIRED -> ROLLING_BACK (no-op) -> ROLLED_BACK
           //     (ruta declarada C3 de §20 con 0 nodos a restaurar)
           //   - >0 nodos mutados -> ROLLBACK_REQUIRED (rollback real desde authorized_plan.json)
           //   (DurableJournalFlushError, §24; nunca COMMITTED)
    ...
    7. Confirmación en Journal y Cierre de Handle:
       Registrar en journal: MUTATED(K).
       if FlushFileBuffers(journal_handle) == FALSE:
           err = GetLastError()
           // El nodo K YA mutó (SetSecurityInfo corrió), pero MUTATED(K) no es durable:
           // un crash dejaría al nodo en la ambigüedad de C4b (§20). Fail-closed:
           CloseHandle(h) -> FAIL_CLOSED -> ROLLBACK_REQUIRED
           (restauración exacta desde authorized_plan.json, §9.2.8; nunca COMMITTED).
       CloseHandle(h).
...
Rollback por nodo (Top-Down, ver §17): el helper usa el `pre_sd_bytes_b64` validado en el paso 3 —cuya copia autoritativa y durable vive en `authorized_plan.json` (§12.2 paso 7); el staging nunca es fuente de rollback, §9.2.8— (no vuelve a leer del filesystem) y reconstruye los componentes del `SECURITY_DESCRIPTOR` self-relative antes de invocar `SetSecurityInfo` (que **no** acepta un buffer SD completo, sino punteros separados a owner/group/dacl):
...
4. **Soporte de Elevación Over-The-Shoulder (OTS) — alcance normativo:** Al residir el staging, el store autorizado y el TGR en `%ProgramData%`, el helper elevado puede leer el plan candidate, el TGR y escribir el plan autoritativo independientemente de la cuenta administrativa ingresada en el diálogo UAC. El acceso al store NO es el problema en OTS; el **token del operador** (§12.3/§21/§22) sí lo es, y el soporte se define exactamente por dos casos con resultados normativos distintos:
   - **Definiciones y resultados normativos:**
     ```text
     OTS_SAME_ACCOUNT  = la cuenta ingresada en el diálogo UAC es la MISMA del operador
                         interactivo (elevación por consentimiento de su propia cuenta admin)
       -> path 1 permitido: la operación completa (mutación + post-verificación) procede.
     OTS_CROSS_ACCOUNT = la cuenta ingresada en el diálogo UAC es OTRA cuenta administrativa
                         (p. ej. operador estándar A ingresa credenciales del admin B)
       -> path 1 NO se usa en v1: el token del operador se obtiene EXCLUSIVAMENTE por la vía
   ...
   - **Obtención del token del operador original (normativa):** la post-verificación (§12.3/§21/§22), el observation pass TOFU y el fresh RV-2 rerun del admission/refresco del TGR (§11.4) deben correr bajo el **token efectivo no elevado del operador interactivo original** (para que `state == HARDENED` valga bajo `CURRENT_EFFECTIVE_UNELEVATED_TOKEN`), que en `OTS_CROSS_ACCOUNT` **no** es la cuenta del diálogo UAC. Un proceso elevado sólo por UAC **no** posee ese token automáticamente. El diseño lo obtiene por una de dos vías:
     1. **El helper elevado EXTRAE el token del coordinador — SÓLO `OTS_SAME_ACCOUNT`:** el helper se eleva por UAC (`ShellExecuteExW` verbo `runas`, §13.1) a integridad **alta**; el coordinador corre a integridad **media**. **Fundamento técnico (evidencia primaria, §29):** el acceso se sostiene en el **DACL check** del objeto proceso — la DACL del SD por defecto de un proceso proviene del token del creador ("The ACLs in the default security descriptor for a process come from the primary or impersonation token of the creator"), que concede los derechos a la propia cuenta del operador — y en que `OpenProcess` evalúa el acceso contra esa DACL. La superioridad de integridad (alto > medio) **no es un sustituto del DACL check**: MIC es "in addition to discretionary access control" y se evalúa *antes* del DACL check, no en su lugar (doc. primaria MIC). Con la DACL garantizada por same-account, el **helper (misma cuenta, integridad alta)** abre el proceso **coordinador** —cuyo `PID` + `ProcessCreationTime` conoce por el contexto de la operación y **verifica** (imagen firmada esperada, misma defensa PID-reuse que §19.1.5)— con `PROCESS_QUERY_INFORMATION | PROCESS_DUP_HANDLE`, hace `OpenProcessToken` + `DuplicateTokenEx` para obtener el token del operador. **Contrato normativo del token generado (evidencia primaria, §29): `CreateProcessWithTokenW` exige un PRIMARY token** cuyo handle posea los derechos de acceso `TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY`; por tanto:
        ...
        DuplicateTokenEx(
            token_del_coordinador,                     // fuente: OpenProcessToken (requiere TOKEN_DUPLICATE)
            TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY,  // desired_access requerido por CreateProcessWithTokenW
            NULL,
            SecurityImpersonation,                     // ignorado por DuplicateTokenEx si TokenType == TokenPrimary (doc. primaria)
            TokenPrimary,                              // NUNCA TokenImpersonation como token pasado a CreateProcessWithTokenW
            &operator_primary_token
        )
        ```
        Un token incompatible (impersonation token o handle sin los derechos requeridos) -> FAIL CLOSED -> `REFUSE_TO_PLAN`: la obtención del token ocurre ANTES de mutar, así que el fallo es pre-mutation (cero nodos mutados, sin `MUTATING(1)`). Con `operator_primary_token` el helper lanza el verificador según la API normativa de §12.3 (`CreateProcessWithTokenW` porque este helper es UAC-elevado y sólo tiene `SeImpersonatePrivilege`; `CreateProcessAsUserW` sólo si el helper corre como SYSTEM/servicio). **No** se intenta la dirección inversa (el coordinador medio no puede abrir el helper alto; y `ShellExecuteExW` no propaga `bInheritHandles`). No requiere `SYSTEM` ni `SeTcbPrivilege`.
     2. **`WTSQueryUserToken` por el componente privilegiado — válida en ambos casos; es la ÚNICA vía en `OTS_CROSS_ACCOUNT`:** si la operación la conduce el servicio privilegiado de Sky-Claw (que arranca como `SYSTEM`/con `SeTcbPrivilege`, ver §11.3), éste obtiene el token del operador con `WTSQueryUserToken(interactive_session_id)`.
   - **Por qué `SeDebugPrivilege` se rechaza en v1 (considerado y descartado):** es la vía documentada para acceso pleno cross-account ("To open a handle to another process and obtain full access rights, you must enable the SeDebugPrivilege privilege", §29), pero habilitarlo otorgaría al helper acceso pleno a *todo* proceso del sistema —mucho más allá del mínimo necesario para extraer un token— y existe una alternativa fail-closed (vía SYSTEM/servicio o `REFUSE_TO_PLAN`). GP2 v1 elige least privilege + fail-closed: **nunca** habilita `SeDebugPrivilege`. Queda prohibida toda claim de "FULL OTS SUPPORT": el soporte OTS garantizado cubre `OTS_SAME_ACCOUNT` siempre, y `OTS_CROSS_ACCOUNT` sólo con el servicio privilegiado disponible.
   - **Fail-closed si el token no está disponible:** si en un escenario OTS (cualquiera de los dos casos) no se puede obtener el token efectivo del operador original, la operación **no** procede a post-verificación bajo un token ajeno; se rehúsa **antes de mutar** (`REFUSE_TO_PLAN`) — la garantía `HARDENED` bajo `CURRENT_EFFECTIVE_UNELEVATED_TOKEN` nunca se evalúa con un token que no representa al operador. `GP2-T22` cubre ambos casos y este desenlace.
...
El flag `SE_DACL_PROTECTED` del descriptor PRE de cada nodo se captura durante la fase de mutación y se persiste en el journal asociado a `MUTATING(K)` (ver §12.2 paso 4). La fuente autoritativa del SD PRE (y del flag) para el rollback y la recuperación es `authorized_plan.json` en `AUTHORIZED_OPERATIONS` (`RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY`, §9.2.8/§12.2 paso 7); el staging nunca es fuente de rollback. El rollback selecciona el flag de `SetSecurityInfo` que se pasa a Windows según ese valor capturado **dentro del bloque normativo completo de §12.2 (rollback)**, que reconstruye `pre_owner`, `pre_group` y `pre_dacl` desde `pre_sd_bytes_b64` y los pasa como parámetros separados:
...
**Caso coherente de ejemplo (corregido; sustituye al ejemplo anterior que describía una DACL sin ACE de `Administrators`, inalcanzable porque la prevalidación de descriptores vivos de §12.2 paso 5 rehúsa ese Golden con `REFUSE_TO_PLAN` antes de cualquier mutación):** el retry con `SeRestorePrivilege` tiene sentido cuando la DACL preexistente **concede** `READ_CONTROL | FILE_READ_ATTRIBUTES` a `Administrators` (la prevalidación §12.2 paso 5 lee el SD vivo con éxito) pero **deniega u omite** `WRITE_DAC`/`WRITE_OWNER` (p. ej. owner = usuario con Owner Rights ACE restrictiva o sin ACE de escritura para `Administrators`): el step-5 live PRE validation funciona, pero el handle de mutación (que necesita `WRITE_DAC | WRITE_OWNER`) obtiene `ERROR_ACCESS_DENIED`, y el retry con `SeRestorePrivilege` (cubierto por `SE_RESTORE_NAME`, ver abajo) reabre con éxito. En coherencia con §12.2 paso 5: **si la prevalidación no puede leer el live SD, el desenlace es `REFUSE_TO_PLAN`** (la excepción de privilegios de §18.2 cubre sólo el probe y el handle de mutación, no la prevalidación).

**`WRITE_OWNER` y la elección de `SeRestorePrivilege` (no `SeTakeOwnershipPrivilege`) — evidencia primaria:** la constante de privilegio **`SE_RESTORE_NAME` (`SeRestorePrivilege`)** está documentada como "grants all *write* access control to any file, regardless of the ACL", y la lista explícita de derechos que concede incluye **`WRITE_DAC` y `WRITE_OWNER`** (además de `FILE_ADD_FILE`, `FILE_ADD_SUBDIRECTORY`, `DELETE`, etc.). Por tanto, abrir el handle de mutación con `READ_CONTROL | WRITE_DAC | WRITE_OWNER` bajo `FILE_FLAG_BACKUP_SEMANTICS` **sí** queda cubierto por el retry con `SeRestorePrivilege` cuando la DACL preexistente lo deniega (el caso coherente y sus límites están definidos en el párrafo anterior: la prevalidación §12.2 paso 5 debe poder leer el live SD; si no, `REFUSE_TO_PLAN`). La elección de `SeRestorePrivilege` es además la **única** correcta para el rollback: restaurar el `OWNER` al SID **original** (que puede no ser el del helper) es una operación de *restore* que exige `SeRestorePrivilege`; `SeTakeOwnershipPrivilege` sólo permite fijar el owner **a uno mismo** (SID del token), por lo que sería insuficiente para restaurar un owner ajeno — y está prohibido en v1 (§8.3/§18). No hay, por tanto, contradicción: el contrato de apertura (`WRITE_OWNER`) y la política de privilegios (`SeRestorePrivilege`) son coherentes y suficientes. La semántica exacta se ancla además con `GP2-T37` (rollback restaura OWNER/GROUP) sobre un fixture cuyo owner es un SID distinto del helper.
...
**Nota sobre `ARCHIVING_BACKUP`:** la fase `ARCHIVING_BACKUP` puede fallar transitoriamente (espacio en disco, antivirus). La política de reintentos es **acotada a nivel de fase** (reintentos dentro de `ARCHIVING_BACKUP` hasta un máximo configurable `MAX_ARCHIVE_RETRIES`); si los reintentos se agotan, la transición es a `ROLLBACK_REQUIRED` (no a un estado huérfano `RETRY_ARCHIVE`). El lock se mantiene durante los reintentos. El `FlushFileBuffers` del manifiesto archivado es parte del intento de archivo: si retorna `FALSE`, el intento cuenta como fallido dentro de `MAX_ARCHIVE_RETRIES` (nunca se transiciona a `COMMITTED` con un respaldo no flusheado); la fuente de re-copia en C7 (§20) es `authorized_plan.json` (§9.2.8). `GP2-T15` cubre el caso de archivo exitoso y de archivo fallido tras agotar reintentos.
...
| **C4: Durante mutación de Nodo K (tras MUTATING(K), antes de MUTATED(K))** | Journal con `MUTATING(K)` y `FlushFileBuffers` persistido | `ROLLBACK_REQUIRED` | Helper re-escanea nodos del plan (nodos 1..K); **primero** revalida `(VolumeSerialNumber, FileId)` esperado vs actual (binding de identidad, vía `GetFileInformationByHandleEx`) y **sólo después** compara bytes del SD actual contra el backup PRE de `authorized_plan.json` (fuente autoritativa, §9.2.8) para decidir si restaurar. Si `FileId` difiere -> `INDETERMINATE` y no se aplica SD. La comparación de bytes SD contra backup es una **detección de drift de mutación** (no una verificación del backup); §9.2.3 prohíbe usar esa comparación como sanity check del backup durante la mutación normal, pero la autoriza como paso legítimo de recuperación (las dos operaciones son distintas). | No (exige UAC) | Sí (UAC para rollback) |
| **C4b: Crash entre `write()` y `FlushFileBuffers` del journal WAL (antes de `MUTATING(K)` durable)** | `write()` ejecutado pero `FlushFileBuffers` no completado: la intención `MUTATING(K)` puede no estar en disco | `INDETERMINATE` | El journal no es evidencia durable del estado transaccional del nodo K. El recuperador re-escanea el plan con revalidación de identidad `(VolumeSerialNumber, FileId)` por nodo: (a) nodos con `MUTATED(K)` durable -> se tratan como mutados; (b) nodos con `MUTATING(K)` durable -> `ROLLBACK_REQUIRED`; (c) nodos sin registro durable -> comparar SD actual contra el backup PRE de `authorized_plan.json` (§9.2.8) **sólo como detección de drift** tras revalidar identidad; si la evidencia es ambigua -> `INDETERMINATE` con informe forense y lock retenido para inspección. GP2-T15 ancla este desenlace (crash simulado entre `write()` y `flush()` -> `INDETERMINATE`, nunca `COMMITTED` silencioso). | No (exige UAC) | Sí (Alerta Crítica) |
| **C4c: Power loss / crash con `authorized_plan.json` perdido, corrupto o no discoverable (después de que la mutación pudo comenzar)** | El plan autoritativo está missing/corrupt/unreadable o falla la revalidación de `plan_digest`; el resto de la evidencia durable disponible DEBE correlacionarse (lock metadata con `phase != RELEASED`, journal parcial, `operations\<op_id>\` existente, FSM, bytes del SD actual del Golden, demás evidencia durable) — un `operations\<op_id>\` vacío NO se interpreta por sí solo de una única manera | `INDETERMINATE` | NO rollback automático (`AUTO_ROLLBACK = NO`: el PRE autoritativo no es demostrablemente recuperable); NO staging fallback (`STAGING_AS_RECOVERY_SOURCE = NEVER`); NO `COMMITTED` (`AUTO_COMMIT = NO`); preservar evidencia e informe forense; intervención del operador obligatoria (`OPERATOR_INTERVENTION_REQUIRED = YES`). Prohibido inventar PRE desde el filesystem actual o reconstruir PRE desde staging. Si un evento catastrófico eliminó TODA la evidencia durable, el sistema tampoco puede demostrar retrospectivamente `MUTATION_NEVER_OCCURRED` (limitación declarada). Oráculo `GP2-T52` / mutante `M-P2` | No | Sí (Alerta Crítica) |
...
| **C7: Durante ARCHIVING_BACKUP (antes de COMMITTED)** | Verificaciones OK en journal, archivo en `golden_backups` parcial | `ARCHIVING_BACKUP` | Re-copiar el manifiesto desde `authorized_plan.json` (fuente autoritativa, §9.2.8) a `golden_backups` de forma segura (con `FlushFileBuffers` verificado, §19.2) y transicionar a `COMMITTED`. Liberar lock. | No (exige UAC) | Sí (UAC) |
| **C8: Durante ejecución de Rollback** | Journal `ROLLING_BACK`, lista parcial de restaurados | `ROLLING_BACK` | Helper retoma rollback para los nodos restantes pendientes de restauración, leyendo el PRE de `authorized_plan.json` (§9.2.8). | No (exige UAC) | Sí (UAC) |
...
5. **Distinción de fuentes de recuperación (normativa):** el respaldo de `golden_backups` es la copia de evidencia persistida a partir de `ARCHIVING_BACKUP`/`COMMITTED`; la fuente autoritativa de restauración **durante** la operación (rollback, recuperación de crash C4–C8) es `authorized_plan.json` en `AUTHORIZED_OPERATIONS` (`RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY`; `UNTRUSTED_STAGING_AS_RECOVERY_SOURCE = NEVER`, §9.2.8). El respaldo archivado nunca es fuente de rollback de GP2 v1 y el staging jamás lo es.
...
| `DurableJournalFlushError` | `FlushFileBuffers` retorna `FALSE` en un gate de durabilidad (WAL `MUTATING(K)`/`MUTATED(K)`, FULL AUTHORIZED PLAN o archivado) | Fail-closed: `SetSecurityInfo` del nodo afectado NO se ejecuta y el FSM no avanza como durable; `ROLLBACK_REQUIRED` (rollback vacío si 0 nodos mutados -> `ROLLED_BACK`, ruta declarada C3 de §20); nunca `COMMITTED` (ver §12.2 pasos 4/7 y §19.2) |
...
- **GP2-T22:** El store de operaciones y backup en `%ProgramData%` permite resolver el plan de forma transparente y autenticar el respaldo en escenarios de elevación Over-The-Shoulder (OTS). **Alcance normativo OTS (§13.4):** (a) `OTS_SAME_ACCOUNT`: path 1 (extracción del token del coordinador) funciona y la operación completa llega a `HARDENED`; **subvariante del contrato del token (§13.4): el `DuplicateTokenEx` del path 1 produce `TokenType == TokenPrimary` con `TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY` y ese primary token es el que recibe `CreateProcessWithTokenW`; un token `TokenImpersonation` (mutante) o sin los derechos requeridos es detectado y produce fail-closed pre-mutation (`REFUSE_TO_PLAN`, sin `MUTATING(1)`)**; (b) `OTS_CROSS_ACCOUNT` con servicio privilegiado disponible: `WTSQueryUserToken` obtiene el token del operador y la operación procede; (c) `OTS_CROSS_ACCOUNT` sin servicio disponible: desenlace `REFUSE_TO_PLAN` **antes de mutar** (cero nodos mutados, sin `MUTATING(1)`); (d) ancla: en cross-account el path 1 **no** se intenta y `SeDebugPrivilege` nunca se habilita.
...
- **GP2-T44 (Golden Admission y refresco del TGR, §11.4):** (a) GP2 apply nunca escribe `trusted_goldens.json`; (b) en `INDEPENDENT_PROVENANCE`, la expectativa completa (TreeDigest + runtime + physical binding; critical expectations si las hay) se valida desde provenance independiente, RV-2 se ejecuta bajo el token original y exige `VERIFIED` antes de la confirmación final y TGR update; (c) en `OPERATOR_TOFU`, first pass sólo produce `OBSERVED`, confirmación privilegiada admite exactamente B y crea receipt helper-issued, luego un **segundo RV-2 fresh desde cero** bajo el token original exige `B2 == admitted B` y un `FreshRuntimeObservation` reacquired desde el root físico que concuerde con `receipt.expected_runtime` antes del TGR update; (d) digest/RV-2 staged por sí solos nunca autorizan; (e) observation por sí sola nunca autoriza; (f) UI/receipt declara `OPERATOR_TOFU_DOES_NOT_DETECT_PRE_EXISTING_COMPROMISE`; (g) el TGR registra el último snapshot autorizado y re-verificado, no una afirmación de current filesystem; GP2 rechaza el tree vivo ante mismatch; (h) si GUI y LLM exponen admission, ambos llaman al mismo backend/DTO y no duplican token/receipt logic; (i) en ambos modos `observed_runtime` viene de un `FreshRuntimeObservation` reacquired desde el root físico en cada pass, nunca de staging/caller/expected_runtime; (j) el digest de critical expectations cumple la serialización exacta de §11.4, incluido `SHA-256(UTF-8("[]"))`; (k) el receipt se traza hasta el TGR por el registro protegido que conserva el receipt y las filas exactas before/after, con `registered_by=operator_sid` y `registered_at` único por root físico. En ambos modos el replace del archivo TGR es atómico a nivel de archivo (sin JSON parcial) y un caller no elevado recibe `ACCESS_DENIED`; no afirma serialización lógica de RMW concurrentes. Un writer concurrente sigue bloqueado por el stop condition de §11.4 hasta tener un mecanismo cross-process probado. Complementa a `M-T2` y a RVO-01..RVO-12.
...
- **GP2-T48 (control handle preexistence, §4.1.4):** fixture que abre sobre un nodo un handle con `WRITE_DAC | WRITE_OWNER` (sin clases de datos, `dwShareMode` generoso) antes del primer probe; aserta (a) el probe **no** produce `ERROR_SHARING_VIOLATION` y la operación llega a `COMMITTED`; (b) post-`COMMITTED`, `SetSecurityInfo` a través del handle preexistente con una DACL permisiva **tiene éxito** (verifica la limitación declarada `CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED`; NO debe intentar "cerrarla" aparentando una mitigación inexistente); (c) el caso gemelo con `dwShareMode = 0` para el handle de control SÍ colisiona con el probe (dirección 1, §4.1.1). Complementa a `M-C1`.
- **GP2-T49 (PRE durability: recovery sin staging, §9.2.8 / §12.2 paso 7):** se completa la autorización (`authorized_plan.json` FULL duradero), se mutan los nodos 1..K y se simula crash; después **se elimina/adultera todo `staging\<op_id>\`**; la recuperación debe: revalidar `plan_digest`/binding de `authorized_plan.json`, revalidar identidad `(VolumeSerialNumber, FileId)` por nodo y ejecutar el rollback exacto desde el PRE protegido **sin leer ningún dato de recuperación del staging** (ancla de comportamiento: el recovery no abre `candidate_manifest.json` como fuente); desenlace terminal declarado, nunca `COMMITTED` silencioso. Cubre el oracle documental: crash mid-mutation + staging eliminado/adulterado -> recovery usa exclusivamente el PRE protegido -> rollback posible. La garantía de T49 presupone `authorized_plan.json` **discoverable + valid**; la variante power-loss con el plan perdido/corrupto es la fila C4c de §20 y `GP2-T52`. Complementa a `M-B3`.
...
- **RVO-09:** `expected_runtime` procede del mismo authority bundle/receipt que `expected_tree`; `observed_runtime` se reacquire con `FreshRuntimeObservation` desde el root físico revalidado en cada pass `OBSERVE`/`VERIFY`, ligado a operation/nonce/root. Runtime staged, runtime de `GoldenMasterCandidate`, copiar `expected_runtime`, o ausencia de observer falla cerrado.
- **RVO-10:** critical expectations, vacías o no, no sustituyen al full TreeDigest y su digest de lista queda ligado al mismo source. El digest usa la serialización canónica exacta de §11.4 (orden de entries, lower-case SHA-256, `expected_size` explícito/null y `[]` para vacío); reordenar/no incluir el conjunto vacío no puede crear una expectation distinta silenciosamente.
...
| **M-C1** | GP2 afirma que el probe detecta/excluye handles de control preexistentes (`WRITE_DAC`/`WRITE_OWNER` sin clases de datos) o que el cambio de DACL revoca derechos ya concedidos a un handle abierto | Test `GP2-T48`: el handle de control con share generoso atraviesa el probe, la operación llega a `COMMITTED` y el handle conserva `WRITE_DAC` operativo post-commit (limitación declarada `CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED`, §4.1.4) |
| **M-B3** | El recovery depende del `candidate_manifest.json` de staging, o `authorized_plan.json` no contiene la tabla PRE completa; un staging eliminado/adulterado tras un crash mid-mutation impide el rollback | Test `GP2-T49`: con staging borrado, el recovery restaura exactamente desde `authorized_plan.json` (`RECOVERY_SOURCE = AUTHORIZED_OPERATIONS ONLY`; `UNTRUSTED_STAGING_AS_RECOVERY_SOURCE = NEVER`, §9.2.8) |
| **M-N1** | El bootstrap privilegiado confía en un namespace preexistente (`CreateDirectory` -> `ERROR_ALREADY_EXISTS` -> continúa) sin verificar tipo/owner/reparse ni normalizar, o confía en un `trusted_goldens.json` plantado | Test `GP2-T50`: el trust root atacante no es confiado; el bootstrap normaliza de forma privilegiada o FAIL CLOSED (§11.3, `PRE_PROVISIONING_NAMESPACE_SQUATTING`) |
| **M-F2** | Se ignora el resultado `FALSE` de `FlushFileBuffers` y el helper ejecuta `SetSecurityInfo` o avanza el FSM como si la escritura fuera durable | Test `GP2-T51`: con flush forzado a `FALSE`, el call count de `SetSecurityInfo` es 0 para ese nodo y nunca `COMMITTED` (§12.2 pasos 4/7, `DurableJournalFlushError` §24) |
| **M-O2** | El helper usa path 1 (extracción del token del coordinador) en `OTS_CROSS_ACCOUNT` justificándolo sólo por integridad alta > media, o habilita `SeDebugPrivilege` para lograrlo | Test `GP2-T22` (c): cross-account sin servicio -> `REFUSE_TO_PLAN` antes de mutar; (d): `SeDebugPrivilege` nunca se habilita (§13.4) |
| **M-O3** | El path 1 de `OTS_SAME_ACCOUNT` genera el token del operador con `TokenType == TokenImpersonation` (o sin `TOKEN_QUERY | TOKEN_DUPLICATE | TOKEN_ASSIGN_PRIMARY`) y lo pasa a `CreateProcessWithTokenW` | Test `GP2-T22` (a): exige `OPERATOR_VERIFIER_TOKEN_TYPE = TokenPrimary` con los derechos requeridos por `CreateProcessWithTokenW`; un token impersonation/incompatible produce fail-closed pre-mutation (`REFUSE_TO_PLAN`, §13.4) |
| **M-P2** | El recovery interpreta `authorized_plan.json` ausente/corrupto como "la operación nunca comenzó" (emite rollback, `ROLLED_BACK` o `COMMITTED` sin plan autoritativo), o intenta recuperar desde `candidate_manifest.json` | Test `GP2-T52`: exige `INDETERMINATE`, 0 llamadas de rollback, 0 lecturas de staging como fuente de recuperación, `COMMITTED` no emitido, evidencia preservada e intervención del operador (fila C4c de §20) |
...
2. **No Atomicidad a Nivel de Sistema de Archivos:** NTFS no provee transacciones multi-archivo (tras la obsolescencia de TxF). La recuperabilidad de GP2 es de nivel aplicativo mediante journal y rollback compensatorio con protocolo WAL. Un fallo catastrófico de energía durante una mutación individual es recuperado mediante el análisis del journal en el siguiente arranque SÓLO SI la evidencia autoritativa requerida (el `authorized_plan.json` FULL y el journal de `AUTHORIZED_OPERATIONS`) sigue discoverable y válida tras el reinicio; si el plan autoritativo se perdió y existe evidencia de operación iniciada, el desenlace obligatorio es `INDETERMINATE` (fila C4c de §20), nunca rollback automático ni `COMMITTED` silencioso.
...
4. **Handles de control preexistentes (`CONTROL_HANDLE_PREEXISTENCE_LIMITATION = DECLARED`):** un handle abierto **antes** de GP2 que posea `WRITE_DAC`/`WRITE_OWNER` (sin clases de datos) con share mode generoso no es detectable por el probe (la compartición sólo evalúa clases de datos, §4.1.1) y conserva los derechos concedidos: el cambio de DACL no revoca el acceso ya concedido a handles abiertos (§4.1.4). Un Actor D puede deshacer el blindaje post-`COMMITTED` a través de ese handle sin abrir handles nuevos. No existe primitiva Win32 soportada para enumerar/revocar estos handles; `HARDENED` se acota al `AccessCheck` fresco de post-verificación (§21). Oracle: `GP2-T48` / mutante `M-C1`.
5. **Durabilidad de la entrada de directorio (`POWER_LOSS_DIRECTORY_ENTRY_DURABILITY_LIMITATION = DECLARED`):** Win32 no documenta un equivalente soportado a `fsync(parent_directory)` — la página "Directory Handles" enumera de forma cerrada las funciones que aceptan un directory handle y `FlushFileBuffers` no está incluida (§29). GP2 v1 garantiza la durabilidad del **contenido** de `authorized_plan.json` (y del journal) mediante `FILE_FLAG_WRITE_THROUGH` + `FlushFileBuffers(handle) == TRUE` (contrato §12.2 pasos 4/7), pero NO afirma una primitiva de sincronización de la entrada de directorio controlable por la aplicación. El modelo de crash se delimita en §12.2 paso 7d y es coherente con la limitación 2 (No Atomicidad a Nivel de Sistema de Archivos):

```text
POWER_LOSS_DIRECTORY_ENTRY_DURABILITY_LIMITATION = DECLARED

Si una pérdida abrupta de energía hace perder la directory entry
del authoritative plan después de que la mutación pudo comenzar,
GP2 v1 NO garantiza rollback automático exacto.
Si existe evidencia de operación iniciada: INDETERMINATE.
...
- `FlushFileBuffers` function (documentada para *a handle to the open file*; NO está incluida en la lista de funciones que aceptan directory handles — sin equivalente Win32 documentado a `fsync(parent_directory)`) — *learn.microsoft.com/windows/win32/api/fileapi/nf-fileapi-flushfilebuffers*
- Directory Handles ("Obtaining a Handle to a Directory": lista cerrada de funciones que aceptan un directory handle — `BackupRead/BackupSeek/BackupWrite`, `GetFileInformationByHandle`, `GetFileSize`, `GetFileTime`, `GetFileType`, `ReadDirectoryChangesW`, `SetFileTime`; `FlushFileBuffers` no está incluida) — *learn.microsoft.com/windows/win32/fileio/obtaining-a-handle-to-a-directory*
...
- `IoCheckShareAccess` routine (share-access sólo se evalúa para clases de datos read/write/delete; `READ_CONTROL`/`WRITE_DAC`/`WRITE_OWNER` no participan) — *learn.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-iocheckshareaccess*
...
- `CreateProcessWithTokenW` (launch under the operator token from a UAC-elevated helper; requires only `SeImpersonatePrivilege`; does not propagate handle inheritance; **hToken must be a PRIMARY token whose handle has `TOKEN_QUERY`, `TOKEN_DUPLICATE` and `TOKEN_ASSIGN_PRIMARY`**) — *learn.microsoft.com/windows/win32/api/winbase/nf-winbase-createprocesswithtokenw*
...
- `OpenProcessToken` + `DuplicateTokenEx` (elevated helper extracts the coordinator's token; el acceso al objeto proceso se gobierna por el DACL check —MIC no lo sustituye—, ver §13.4; **`TokenType = TokenPrimary` produce un primary token utilizable por `CreateProcessAsUserW`/`CreateProcessWithTokenW`, e `ImpersonationLevel` es ignorado cuando `TokenType == TokenPrimary`**) / Mandatory Integrity Control — *learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-openprocesstoken* · *learn.microsoft.com/windows/win32/api/securitybaseapi/nf-securitybaseapi-duplicatetokenex* · *learn.microsoft.com/windows/win32/secauthz/mandatory-integrity-control*
- Access Rights for Access-Token Objects (`TOKEN_QUERY`, `TOKEN_DUPLICATE`, `TOKEN_ASSIGN_PRIMARY` y el resto de derechos de token) — *learn.microsoft.com/windows/win32/secauthz/access-rights-for-access-token-objects*
...
- `CreateFileW` `FILE_FLAG_WRITE_THROUGH` (writes bypass intermediate cache; *"a write-through request via FILE_FLAG_WRITE_THROUGH also causes NTFS to flush any metadata changes ... that result from processing the request"*) — *learn.microsoft.com/windows/win32/api/fileapi/nf-fileapi-createfilew* (dwFlagsAndAttributes; Remarks: Caching Behavior)
- Keith Brown, *MSDN Magazine* (archivo Microsoft), "Security Briefs: Exploring Handle Security in Windows" — el security descriptor se evalúa sólo al abrir el handle; *"changes to the security descriptor of an object have no effect whatsoever on existing sessions (handles) to that object"* — *learn.microsoft.com/en-us/archive/msdn-magazine/2000/march/security-briefs-exploring-handle-security-in-windows*
- Process Security and Access Rights — la DACL del SD por defecto de un proceso proviene del token del creador; `OpenProcess` evalúa el acceso contra esa DACL; *"To open a handle to another process and obtain full access rights, you must enable the SeDebugPrivilege privilege"* — *learn.microsoft.com/windows/win32/procthread/process-security-and-access-rights*
- Mandatory Integrity Control — MIC es *"in addition to discretionary access control"* y se evalúa *antes* del DACL check (no lo sustituye) — *learn.microsoft.com/windows/win32/secauthz/mandatory-integrity-control*
