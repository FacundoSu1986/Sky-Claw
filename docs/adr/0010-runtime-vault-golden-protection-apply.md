...
| **C3: UAC aceptado, antes del Nodo 1** | Journal `APPLYING` en authorized store, lista de nodos mutados vacía | `APPLYING` (0 nodes) | **Recuperación read-only, sin escritura autoritativa:** el coordinador no elevado **lee** el journal (permitido por la DACL de `operations/`, §11.3: `FILE_GENERIC_READ`), confirma 0 nodos `MUTATED` y **abandona** ese `op_id` (ninguna mutación ocurrió; el `GoldenMutationLock` ya lo liberó el kernel al morir el helper). NO se escribe `CANCELLED` en el journal autoritativo desde un proceso no elevado (respeta `WHO_WRITES_AUTHORITATIVE_FSM`, §19.2): el registro `APPLYING` queda **inerte** (read-only) y no afecta operaciones futuras (cada operación usa un `op_id` nuevo); su limpieza opcional la hace el bootstrap de un componente privilegiado. No requiere UAC porque no hay escritura autoritativa ni cambio en el FS. | Sí | No |
