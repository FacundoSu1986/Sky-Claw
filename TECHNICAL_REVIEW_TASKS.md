...
- **Test rojo:** dos procesos con instancias independientes de `DistributedLockManager` sobre la misma SQLite, instalando en el mismo directorio, se serializan: el segundo devuelve `already_existed=True`, sin segunda descarga ni segundo HITL. (Dos corrutinas en un proceso no prueban nada cross-process.) Sumar cobertura de expiración del lease, renovación y excepción a mitad de instalación.
- **Ancla que enumera (no muestrea):** la familia de mutadores de `ToolsInstaller` se detecta por introspección y se congela, al estilo de `tests/test_hitl_client_scoping.py` — un `ensure_*` nuevo rompe el ancla hasta que se le escribe su receta. Debe verificar **una adquisición por recurso** en cada ruta, desde el chequeo de existencia hasta la limpieza, sin adquisiciones duplicadas en las llamadas indirectas. *La propiedad, no el recordatorio: el lock cross-process solo protege si TODOS los mutadores participan.*
