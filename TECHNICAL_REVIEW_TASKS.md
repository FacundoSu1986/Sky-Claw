...
- **Problema:** `ToolsInstaller` no tiene lock, journal ni preflight. Es la clase "dos superficies, un recurso" de `AGENTS.md`: el agente LLM alcanza las rutas de instalación y la GUI alcanza LOOT/SSEEdit/Pandora. Dos instalaciones concurrentes sobre el mismo `install_dir`/`mod_dir` intercalan chequeo-de-existencia, extracción, renombrado, verificación de payload, `meta.ini` y `unlink` del archive — con corrupción silenciosa del directorio como resultado.
- **La familia COMPLETA de mutadores** (siete, no seis — la enumeración original de este ítem ya se había dejado uno afuera, que es precisamente el defecto que la tarea existe para cerrar):
  `ensure_loot`, `ensure_xedit`, `ensure_pandora`, `ensure_bodyslide`, `ensure_ngio`, `_ensure_github_mod`, `_ensure_nexus_mod`.
  `ensure_ngio` es pública y de cara al agente LLM, e instala **tres** componentes delegando en `_ensure_github_mod`/`_ensure_nexus_mod`. Necesita receta propia: toma un lock **por cada `mod_dir`** (no uno global — un componente no debe frenar a otro), y las llamadas delegadas no pueden re-adquirir el lock que el caller ya tiene (reentrada → deadlock o doble adquisición).
...
- **`resource_id` canónico:** `PathValidator.validate()` solo resuelve la ruta, **no** normaliza mayúsculas. Derivar la clave del path validado con `os.path.normcase(...).casefold()`, igual que `vfs_instance_id()` — si no, `C:\Tools\LOOT` y `c:\tools\loot` toman locks distintos sobre el mismo directorio y el lock no protege nada. Cubrir en tests las grafías relativa, absoluta, UNC y con distinta capitalización.
- **Lease y espera:** `DistributedLockManager` expira por TTL y reintenta acotado (`LockAcquisitionError` al agotar). Una descarga + extracción puede tardar hasta `_SEVENZIP_TIMEOUT_SECONDS` (300 s) **más** validación y limpieza: definir TTL mayor al peor caso o renovar el lease, y una espera total que permita al segundo llamador ver `already_existed=True` en vez de fallar. Liberar **siempre** en `finally`, incluido error de instalación y cancelación.
- **Test rojo (contención real, no secuencia accidental):** dos procesos con instancias independientes de `DistributedLockManager` sobre la misma SQLite. El primero adquiere el lock y **espera en una barrera antes de mutar**; el segundo intenta adquirir el mismo recurso y debe quedar **bloqueado hasta la liberación** — recién entonces devuelve `already_existed=True`, sin segunda descarga ni segundo HITL. Sin la barrera el test pasa aunque el segundo proceso simplemente haya arrancado después, que no demuestra nada. (Dos corrutinas en un proceso tampoco prueban nada cross-process.) Sumar cobertura de expiración del lease, renovación y excepción a mitad de instalación.
- **Test de alcance (que el lock cubra TODO el ciclo):** con un fake del lock que registre cada fase, verificar que sigue adquirido durante el chequeo de existencia, la extracción, la validación y la limpieza — la introspección sola no puede demostrarlo, solo enumera.
- **Ancla que enumera (no muestrea):** la familia de mutadores de `ToolsInstaller` se detecta por introspección y se congela, al estilo de `tests/test_hitl_client_scoping.py` — un `ensure_*` nuevo rompe el ancla hasta que se le escribe su receta. La introspección cubre **qué rutas** participan y que no haya adquisiciones duplicadas en las llamadas indirectas; **cuánto dura** el lock lo cubre el test de alcance de arriba. *La propiedad, no el recordatorio: el lock cross-process solo protege si TODOS los mutadores participan.*
- **Aceptación:** las siete rutas participan del mismo lock; ancla por introspección en verde; `docs/pending_ooda_status.md` actualizado en el mismo PR.
