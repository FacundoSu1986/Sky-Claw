...
async def first_run_wizard():
    ...
    provider = (
        input(
            f"Proveedor de LLM (anthropic/openai/deepseek/ollama) [{config.llm_provider}]: "
        )
        .strip()
        .lower()
    ...
    if provider == "openai":
        api_key = (
            input(f"API Key para OpenAI [{config.openai_api_key}]: ").strip()
            or config.openai_api_key
        )
        model = (
            input(f"Modelo (ej: gpt-4o) [{config.llm_model}]: ").strip()
            or config.llm_model
            or "gpt-4o"
        )
    elif provider == "deepseek":
        api_key = (
            input(f"API Key para DeepSeek [{config.deepseek_api_key}]: ").strip()
            or config.deepseek_api_key
        )
        model = (
            input(f"Modelo (ej: deepseek-chat) [{config.llm_model}]: ").strip()
            or config.llm_model
            or "deepseek-chat"
        )
        ...
        api_key = ""
        model = (
            input(f"Modelo (ej: llama3.1) [{config.llm_model}]: ").strip()
            or config.llm_model
            or "llama3.1"
        )
    else:  # anthropic default
        api_key = (
            input(f"API Key para Anthropic [{config.anthropic_api_key}]: ").strip()
            or config.anthropic_api_key
        )
        model = (
            input(
                f"Modelo (ej: claude-3-5-sonnet-20240620) [{config.llm_model}]: "
            ).strip()
            or config.llm_model
    ...
    nexus_key = (
        input(f"API Key de Nexus Mods (opcional) [{config.nexus_api_key}]: ").strip()
    ...
    mo2_root = input(
        f"Ruta de MO2 Root [{detected.get('mo2_root', config.mo2_root)}]: "
    ).strip() or detected.get("mo2_root", config.mo2_root)
    skyrim_path = input(
    ...
    bot_token = (
        input(f"Telegram Bot Token [{config.telegram_bot_token}]: ").strip()
        or config.telegram_bot_token
    )
    chat_id = (
        input(f"Telegram Chat ID [{config.telegram_chat_id}]: ").strip()
