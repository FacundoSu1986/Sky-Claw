...
def _hash_file_sha256(file_path: Path) -> str:
    """Calcula el hash SHA-256 de un archivo en bloques acotados sin cargarlo todo en memoria."""
    ...
    try:
        for child in sorted(mods_dir.iterdir(), key=lambda p: p.name):
            ...
            if marker_candidate.is_file() and validate_path_containment(marker_candidate, mods_dir):
                ...
                st = resolved_marker.stat()
                sha = _hash_file_sha256(resolved_marker)
                markers.append(
...
def run_parallaxr_assisted_preflight(
    ...
    try:
        st = selected_entrypoint.stat()
        sha = _hash_file_sha256(selected_entrypoint)
    ...
    return ParallaxRAssistedPreflight(
        ...
        existing_output_candidates=existing_output,
        candidate_mod_roots=candidate_mod_roots,
        message="Instalación oficial de ParallaxR lista para handoff manual.",
    )
...
def prepare_parallaxr_manual_handoff(
    ...
    Transformación pura: falla cerrado si preflight no está en estado READY.
    """
    if preflight.status != ParallaxRAssistedStatus.READY or preflight.installation is None:
        raise ValueError(
            f"No se puede preparar handoff: preflight no está en estado READY (status={preflight.status.value})."
        )
