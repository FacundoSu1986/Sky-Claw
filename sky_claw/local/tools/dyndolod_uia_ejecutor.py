...
def _respuesta_pertenece_a_la_solicitud(solicitud: SolicitudPreflightUIA, resultado: ResultadoPreflightUIA) -> bool:
    ...
    ``pid`` se exige cuando AMBOS lados lo declaran: un corte temprano del
    pipeline (tool desconocida, sensor caído) deja ``resultado.pid is None``
    y no es de otra solicitud. Un MATCH/MISMATCH con pid distinto sí lo es.
    """
    if resultado.tool != solicitud.tool:
        return False
    if solicitud.pid is not None and resultado.pid is not None and resultado.pid != solicitud.pid:
        return False
