...
3. Success is decided by exit code AND artifact at the `-o:` root (`DynDOLOD.esp` as a file for DynDOLOD) AND no error lines in `Logs/{Tool}_{modo}_log.txt` — never by exit code alone.
