...
3. Success is decided by **four** conjuncts, never by exit code alone: `rc == 0` AND the required artifact PRESENT at the tool's exclusive `-o:` root (`DynDOLOD.esp` as a file for DynDOLOD; `textures/` non-empty for TexGen) AND the tool's **completion marker** in `Logs/{Tool}_{modo}_log.txt` AND **no terminal failure** in that log. **Error lines are NOT the gate** — the earlier rule ("no error lines") rejected every real success: rig 2026-08-10 measured **121 `Error:` lines in a SUCCESSFUL DynDOLOD run**, all of them domain noise. The log is classified into three boxes, not two — completion marker, known non-fatal, terminal — and *when in doubt, terminal*: a non-fatal misread as terminal is a reviewable red, the reverse is a false green. **Severity outranks category:** `Critical:`/`Fatal:`/`FATAL:`/`FAIL:` classify terminal even when the same line also names a known non-fatal category — `Critical: Deleted reference […] is corrupt` aborted the run whatever it was talking about, and evaluating the category box first let it through as noise. `Error:` is deliberately NOT one of those severities (it is the 121 lines), so it stays terminal only when no known category explains it. Anchored by the exhaustive product of every category × every severity plus its counterpart over `Error:` — a category added later joins the product on its own. The lists live in `MARCADORES_DE_COMPLETITUD`, `NO_FATALES_DEL_DOMINIO` and `PATRONES_TERMINALES` (`tools/dyndolod_runner.py`), and the completion markers **differ per tool** — wiring DynDOLOD's and leaving TexGen without its own leaves half the stage with no completeness gate. **A missing or unreadable log now FAILS the run** (it used to be a warning): without a log there is no completion marker, and the artifact gate only proves a valid output EXISTS, not that the run finished. A GUI the operator closed mid-way exits 0 without flushing its log while having touched the tree — exactly the combination the old rule let through. Fail-closed: "I could not verify it finished" is not "it finished". **The marker must belong to THIS run, and the log is signed before launch** — otherwise the completeness conjunct is falsifiable with yesterday's state: a previous successful run leaves its log WITH the marker, today's run writes its output (the born-empty exclusive root makes it legitimately this run's) and dies before rewriting the log. Comparing the log's signature is necessary but NOT sufficient: it proves the FILE changed, not that the marker is new. If the binary appends instead of truncating, an interrupted run appends its startup lines — signature changed — while yesterday's marker survives in the text. **So the marker is searched only in the bytes the run added** (`_validar_completitud_de_la_corrida`). The documented NORMAL semantics is append between sessions (dyndolod.info/Messages: "All messages are appended … when the tool is closed"; only the debug log is rewritten per session), so slicing is the right read of the ordinary case. **But the append has to be PROVEN, not assumed, and `mtime + size` cannot prove it:** a truncate-and-rewrite that ends LARGER than the previous file has exactly the same signature as an append — both grow, both change mtime. Slicing on that assumption drops bytes the CURRENT run wrote, and if a `Fatal:` is among them the verdict comes out green on a run that aborted. That was measured, not hypothesized. **So the pre-launch signature is `(size, sha256 of the content)` and the prefix is RE-HASHED after the run** (`_firma_del_log`, `_digest_del_prefijo`): slicing `bytes[:previous_size]` is legal only when those bytes are still byte-for-byte what they were before launch. Four outcomes, none of them guessed — no log before: the whole file is this run's; grew and the prefix matches: append PROVEN, evidence is the tail; grew and the prefix does NOT match: rewritten, not appended, fail-closed; same size or smaller: no demonstrable new bytes, fail-closed (a fresh mtime over an unchanged size proves nothing). Hashing costs two streaming passes of ~1 MiB chunks — measured at ~190 ms over a 100 MiB log, against a run that takes hours — and never holds the file in memory. Terminal lines are scoped by the SAME window: a terminal from a failed previous session sits in the prefix of an appending log forever and would red every later GOOD run until the operator truncates it by hand. Terminals and marker are both classified over the bytes this run added; when the boundary is indeterminate, the run fails with that reason and NO historical line is attributed to it. The three properties hold together, and each has its own anchor: an inherited terminal does not sink a good run, an inherited marker does not approve one, and a CURRENT terminal can never fall outside the evidence because an append was assumed without proof. **The two residuals this section used to declare — a log whose mtime changed without growing, and a replacement that ends larger — are CLOSED, and closed with tests rather than with a paragraph** (`test_el_mtime_sin_crecimiento_no_es_evidencia_de_esta_corrida`, `test_una_reescritura_que_crece_no_se_toma_por_append`, `test_un_log_mas_chico_que_antes_de_lanzar_falla_cerrado`, each parametrized over BOTH launchers). Anchors: `tests/test_dyndolod_taxonomia_log.py` (frozen marker dict + the three boxes, the stale/append pair and its exact counterpart, and the four continuity outcomes — rewrite-grow, real append, terminal in the appended tail, same-size and shrink — each over BOTH launchers) and `test_log_ausente_es_fallo_aunque_el_artefacto_exista`. **Terminal lines are what `not post.terminales` gates, and that conjunct has its own isolated anchor per launcher** (`test_exit_cero_con_error_en_log_no_es_exito` and its `_dyndolod` twin): both write a log carrying the terminal line AND that tool's completion marker, so completeness cannot be what fails them. They did not do that at first — the marker was missing and one of them wrote the *other* tool's log — and deleting `not post.terminales` from both success formulas left the whole suite green. **The artifact freshness comparison was REMOVED by PR-3; per-run log attribution remains.** The old gate compared a pre/post signature of the artifact (mtime + size, or the aggregate tree for TexGen) because the staging was not cleaned or moved aside between runs. Since PR-2 that property is established at the transaction boundary instead: the service moves the tool's WHOLE exclusive root aside (`DirectoryRollback`), recreates it empty and asserts zero entries BEFORE the spawn, with the ownership lease live — so "what is in the root" and "what this run generated" are the same set, and the ∃ predicate ("something changed") is redundant. The completion-marker conjunct carries the weight the old signature never could: a GUI app the operator closed mid-way exits 0, but it did not write its marker, so the run fails closed. The two former limits of the freshness gate are closed by that conjunct, not by the signature: (a) a TexGen run that wrote some textures and aborted has no marker; (b) for DynDOLOD, a plugin persisted early cannot pass without `DynDOLOD plugins generated successfully` / `Occlusion.esp completed successfully`. Anchors: `test_la_machinery_de_frescura_de_artefactos_no_esta_en_el_veredicto` (the retired machinery cannot come back), `test_t1_dyndolod_born_empty_output_y_completitud_es_exito` and `test_t2_texgen_born_empty_texturas_y_completitud_es_exito` (born-empty + artifact + marker), `test_t7_root_no_born_empty_impide_el_spawn` and the T9/T10 pair (sibling or family bytes never count).
4. The administered output root is **per tool** since P2.1 (candidate, PR-2 DRAFT): TexGen receives `-o:<external_work_root>/DynDOLOD/TexGen` and DynDOLOD `-o:<external_work_root>/DynDOLOD/DynDOLOD` — exclusive sibling subroots of the external work root, never the game tree and never the `DynDOLOD/` family namespace (which is not a packageable unit). The legacy `<game>/Sky-Claw/DynDOLOD` is `LEGACY_RECOVERY_ONLY` (recovery reconciler only; no new run writes there). **Changing `-o:` REOPENED the launch gate:** the T5-v2 PASS (2026-09-10) is evidence for the OLD builder/root and does not certify the new one; the two real TexGen/DynDOLOD runs must be repeated on the complete PR-2 candidate before merge. TexGen Alpha-209 was physically observed writing its artifact at `<texgen_root>/textures`; DynDOLOD keeps its separate `DynDOLOD_Output`/root-with-`DynDOLOD.esp` contract. A successful TexGen result therefore carries `<texgen_root>/textures`, while packaging preserves that directory as `mods/TexGen Output/textures/...` so the Data-relative prefix is not lost. The root hangs off the `Sky-Claw/` namespace on purpose: bare `<game>/DynDOLOD` is the folder the DynDOLOD Standalone archive extracts to, so an operator with the tool installed there would get its own installation directory back as `-o:`. Sky-Claw's write-permission preflight probes that root (plus the first existing ancestor, to prove the root can be CREATED on the first run) **and the executable's own directory**, where the tool writes its INI and the `Logs/` that step 3 reads. **The `DynDOLOD/` FAMILY namespace is not a packageable unit:** since P2.1 each tool writes into its own EXCLUSIVE subroot (`DynDOLOD/TexGen`, `DynDOLOD/DynDOLOD`), and the family directory is never emitted as `-o:`; should some run ever write into the family root, detection would still recognise it — the `DynDOLOD.esp` gate is unchanged — but packaging fails closed instead of attributing its children to one tool, because one of those children is TexGen's `textures`. The rule is *shared namespace ≠ packageable namespace*, NOT a filter on the name `textures`: excluding one name would let any other foreign child through. **P2.2 (candidate) raises that boundary to the WHOLE exclusive root per tool**: the move-aside covers `<external>/DynDOLOD/TexGen` or `<external>/DynDOLOD/DynDOLOD` entirely, the root is recreated and verified empty before its spawn, the DynDOLOD root is born-empty even with `run_texgen=False`, and the TexGen root is left byte-exact when that flag omits it. The same candidate wires the live `WorkspaceResuelto` (root + P2.0 ownership lease) into the service: `assert_owned` runs before the move-aside, before creating the empty root, before each spawn and before each packaging, and the workspace lease joins the rollback veto - a lost lease stops every later mutation and forbids restoring a backup over a concurrent owner's output. **P2.2 also fences the PHYSICAL chain (blocker P1):** `links.exigir_contencion_fisica` walks every component from the admitted root to the mutated path with `lstat` (never following links), rejects symlink/junction ancestors and revalidates each captured identity (the classifier covers symlinks and `IO_REPARSE_TAG_MOUNT_POINT`, not every reparse tag Windows may define), right before the move-aside, the born-empty creation, each spawn and each packaging - a junction swapped into an ancestor after boot can no longer redirect a managed mutation outside the workspace even though `resolve()` of both ends points into the same external tree. **P2.3 (candidate) closes the startup side of the same contract:** the boot sweep declares the two raw external tool roots as ACTIVE_TARGETs of the `dyndolod` producer (together with the two packaged mods), keeps the `LEGACY_RECOVERY_ONLY_TARGET` in its own `dyndolod-legacy-recovery` producer under the same ritual lock, and injects the external roots from the durable `RegistroDeRootActivo` (active root plus the `desde` of a pending transition) because the sweep runs BEFORE the workspace is resolved - editing the preference cannot orphan the previous root's backups. A hard death between move-aside/mkdir/copy now restores (target absent) or preserves both (target present), per target. **TexGen's staging is moved aside before the run**, so it is born empty and its contents ARE this run's output — the freshness gate only ever proved that *something* changed, never that the whole tree belonged to the run; its exact destination is declared in `rollback_reconciler` so a hard death does not orphan the backup. **And packaging is not deployment:** Sky-Claw runs standalone, so before spawning DynDOLOD it proves that every file of the fresh TexGen staging is visible under the `-d:<Data>` the binary will actually open, byte for byte (physical identity or sha256, complete over the tree, never sampled). No proof → DynDOLOD is not launched. Sky-Claw does NOT materialise anything on its own: it does not copy into `Data`, edit the modlist or launch `ModOrganizer.exe`. **When that gate cuts, the packaged `TexGen Output` is PRESERVED** while the rest of the run rolls back (the raw staging still reverts — being born empty is B's precondition): the artifact the operator must deploy is exactly the one the old rollback destroyed, which left the GUI default with no way out. The transaction stays PENDING — never ROLLED_BACK — and the log names the surviving directory. The continuation is a second call with `run_texgen=False`, which re-verifies the preserved mod against `Data` byte for byte before spawning DynDOLOD, so recovery never depends on the binary regenerating identical output; with no packaged `TexGen Output` present that flag keeps its pre-existing meaning and is not gated. Anchors: `test_pipeline_no_empaqueta_root_compartido_de_dyndolod`, `test_el_root_compartido_no_es_empaquetable_aunque_no_haya_textures`, `test_el_staging_de_texgen_no_hereda_archivos_de_una_corrida_anterior`, `test_no_se_lanza_dyndolod_si_texgen_no_es_visible_en_data` and its stale/positive counterparts. These three fail-closed boundaries close T3's fallout; they do NOT close T5 — the preset's `OutputPath=` precedence, the CRT/Delphi path serialization and the real DynDOLOD rig run were closed afterwards by the T5-v2 run (see the gate status in point 1), and the per-tool exclusive subroots (the definitive fix for the shared-root class, which changed `-o:`) are IMPLEMENTED AS CANDIDATES in P2.1 (derivation + per-tool `-o:`) and P2.2 (production wiring of the `WorkspaceResuelto`, fences, born-empty of the whole tool root and disjoint packaging); what stays blocked is P2.3 (startup recovery of the external `ACTIVE_TARGET` backups) and the two real rig runs that must be repeated on the complete PR-2 candidate - the launch gate remains REOPENED and PR-2 NOT READY TO MERGE.
