...
2. **Bearer-token injection** — callers that require authentication receive
   the token from :class:`~sky_claw.app.security.auth_token_manager.AuthTokenManager`
...
def authenticated_connect(
    ...
    **connect_kwargs: Any,
    ...
    2. Resolve the bearer token:
       ...
       * Falls back to :meth:`~sky_claw.app.security.auth_token_manager.AuthTokenManager.read_token_file`
         using *token_dir*.
