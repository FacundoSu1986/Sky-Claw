...
async def _run_environment_scan(scanner, store: ReactiveStore) -> None:
    ...
    run. We offload the whole scan onto a daemon worker thread (its own event
    loop) so the UI stays responsive and a blocked filesystem probe cannot
    retain the process after shutdown.
    """
    try:
        snapshot = await run_off_loop(lambda: asyncio.run(scanner.scan()))
    except Exception:
