...
from sky_claw.app.gui.controllers import (
    ChatController,
    ModController,
...
from sky_claw.app.gui.gui_helpers import _load_css
...
def _gui_dir() -> Path:
    ...
    :mod:`sky_claw.app.web.app` and read the bundled assets from
    ``sys._MEIPASS`` instead (these are declared in ``sky_claw.spec`` datas).
    """
    if getattr(sys, "frozen", False):
        return Path(sys._MEIPASS) / "sky_claw" / "app" / "gui"  # type: ignore[attr-defined]
...
def main_page() -> None:
    ...
    def _on_conflict_scan() -> None:
        ...
        from sky_claw.app.core.conflict_persistence import claim_scan_slot
