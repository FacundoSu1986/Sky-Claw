...
class CoreEventBus:
    ...
    def _resolve_handler(self, handler_name: str) -> Subscriber | None:
        """Resuelve identidades DLQ actuales o con un prefijo de módulo anterior."""
        exact = self._handler_index.get(handler_name)
...
def create_bus_with_dlq(db_path: Path | None = None, *, lifecycle=None) -> CoreEventBus:
    ...
    dlq = DLQManager(
        ...
        handler_resolver=scratch_bus._resolve_handler,
        lifecycle=lifecycle,
    )
    bus = CoreEventBus(dlq=dlq, require_dlq=True)
    # Re-direccionar el handler_resolver al índice del bus definitivo.
    dlq._handler_resolver = bus._resolve_handler  # type: ignore[attr-defined]
    return bus
