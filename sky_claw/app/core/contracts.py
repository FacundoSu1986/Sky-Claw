...
Pydantic v2 de sky_claw.app.core.schemas, y decoradores @validate_input,
...
def _ensure_registry() -> None:
    """
    Lazy-init thread-safe: escanea sky_claw.app.core.schemas la primera vez
    ...
    sky_claw.app.core.__init__.py que importa este módulo. La bandera se
    ...
    with _REGISTRY_LOCK:
        ...
        try:
            ...
            _schemas_module = importlib.import_module("sky_claw.app.core.schemas")
            ...
            logger.warning(
                "No se pudo importar sky_claw.app.core.schemas — los decoradores de contratos funcionarán en modo pass-through."
            )
...
def _resolve_schema(name: str | None) -> type[BaseModel] | None:
    ...
    if schema_cls is None and name not in ("dict", "str", "list[dict]"):
        logger.warning(
            ...
            "en SchemaRegistry. Verificar sky_claw.app.core.schemas.",
            name,
        )
    return schema_cls
