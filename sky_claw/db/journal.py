...
class OperationJournal:
        ...
        async with self._lock:
            try:
                cursor = await db.execute(
                    ...
                    """,
                    (TransactionStatus.COMMITTED.value, transaction_id, TransactionStatus.PENDING.value)
                )
                await db.commit()
    ...
    async def begin_operation(
        ...
        async with self._lock:
            try:
                ...
                if entry_id is None:
                    raise JournalTransactionError(
                        "Failed to get entry ID after insert"
                    )
    ...
    async def fail_operation(
        ...
        logger.error(
            "Operation failed",
            extra={"entry_id": entry_id, "error": error}
        )
    ...
    async def mark_rolled_back(
        ...
        logger.info(
            ...
            extra={"entry_id": entry_id, "details": details}
        )
    ...
    async def mark_transaction_rolled_back(
        self, 
        transaction_id: int
    ) -> None:
        """
