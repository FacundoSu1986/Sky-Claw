"""Tests for sky_claw.app.security.auth_token_manager.AuthTokenManager.
...
from sky_claw.app.security.auth_token_manager import _TOKEN_TTL, AuthTokenManager
...
class TestValidateTTLExpiry:
    def test_token_valid_just_before_ttl(self, tmp_path):
        ...
        with patch("sky_claw.app.security.auth_token_manager.restrict_to_owner"):
            with patch("sky_claw.app.security.auth_token_manager.time") as mock_time:
...
class TestTokenFile:
    ...
    def test_generate_calls_restrict_to_owner_on_file(self, tmp_path):
        ...
        with patch("sky_claw.app.security.auth_token_manager.restrict_to_owner") as mock_restrict:
            mgr = AuthTokenManager(token_dir=str(token_dir))
...
class TestTimingSafeComparison:
    ...
    def test_compare_digest_is_called_on_valid_token(self, tmp_path):
        ...
        with patch(
            "sky_claw.app.security.auth_token_manager.secrets.compare_digest",
...
class TestRotationLoopOffloading:
    async def test_rotation_offloads_generate_to_thread(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "sky_claw.app.security.auth_token_manager.asyncio.to_thread",
            fake_to_thread,
        )
        monkeypatch.setattr(
            "sky_claw.app.security.auth_token_manager.asyncio.sleep",
            fake_sleep,
        )
        with (
            patch("sky_claw.app.security.auth_token_manager.restrict_to_owner"),
    ...
    async def test_stop_rotation_waits_for_inflight_generate(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sky_claw.app.security.auth_token_manager._TOKEN_TTL", 0.01)
