...
class TestSearchMod:
    @pytest.mark.asyncio
    async def test_search_mod_no_results(
        self, tool_registry: AsyncToolRegistry
    ) -> None:
        result = json.loads(
            await tool_registry.execute("search_mod", {"mod_name": "SKSE"})
        )
        assert result["matches"] == []

    @pytest.mark.asyncio
    async def test_search_mod_with_results(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_search_mod_partial_match(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
        await adb.upsert_mod(nexus_id=100, name="SkyUI", version="5.2")
        await adb.upsert_mod(nexus_id=101, name="Sky Tweaks", version="1.0")
        result = json.loads(
            await tool_registry.execute("search_mod", {"mod_name": "Sky"})
        )
        assert len(result["matches"]) == 2

    @pytest.mark.asyncio
    async def test_search_mod_empty_name_rejected(
        self, tool_registry: AsyncToolRegistry
    ) -> None:
        with pytest.raises(pydantic.ValidationError):
...
class TestCheckLoadOrder:
    ...
    @pytest.mark.asyncio
    async def test_check_load_order_empty_profile_rejected(
        self, tool_registry: AsyncToolRegistry
    ) -> None:
        with pytest.raises(pydantic.ValidationError):
...
class TestDetectConflicts:
    ...
    @pytest.mark.asyncio
    async def test_detect_missing_master(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
        mod_id = await adb.upsert_mod(
            nexus_id=30150, name="SKSE-30150-v2-2-6", version="2.2.6"
        )
...
class TestRunLootSort:
    @pytest.mark.asyncio
    async def test_run_loot_sort_missing_exe(
        self, tool_registry: AsyncToolRegistry
    ) -> None:
        result = json.loads(
...
class TestInstallMod:
    @pytest.mark.asyncio
    async def test_install_mod_registers(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
        result = json.loads(
            await tool_registry.execute(
                "install_mod", {"nexus_id": 5555, "version": "1.2.3"}
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_install_mod_invalid_params(
        self, tool_registry: AsyncToolRegistry
    ) -> None:
        with pytest.raises(pydantic.ValidationError):
            await tool_registry.execute(
                "install_mod", {"nexus_id": -1, "version": "1.0"}
            )
...
class TestSearchModSpecialChars:
    @pytest.mark.asyncio
    async def test_percent_in_name(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_single_quote_in_name(
        self, tool_registry: AsyncToolRegistry, adb: AsyncModRegistry
    ) -> None:
        await adb.upsert_mod(nexus_id=400, name="Skyrim's Edge", version="1.0")
        result = json.loads(
            await tool_registry.execute("search_mod", {"mod_name": "Skyrim's"})
        )
        assert len(result["matches"]) == 1
...
class TestDetectConflictsLargeDataset:
    @pytest.mark.asyncio
    async def test_500_mods_with_missing_masters(
        ...
        for i in range(500):
            mod_id = await adb.upsert_mod(
                nexus_id=10000 + i, name=f"Mod-{i}", version="1.0"
            )
            await adb.insert_deps_batch([(mod_id, 90000 + i, f"MissingDep-{i}")])

        result = json.loads(
            await registry.execute("detect_conflicts", {"profile": "Default"})
        )
